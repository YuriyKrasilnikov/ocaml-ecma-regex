...
DETAIL_NAME = "ecma262-regexp-match-engine-end-anchor-exact-plan.tsv"
SUMMARY_NAME = "ecma262-regexp-match-engine-end-anchor-exact-plan.summary"
TARGET_TEST_ARTIFACT = "test/test_ecma262_match_engine_end_anchor_exact_plan.ml"
SOURCE_FAILURE_FAMILY = "matcher_runtime_end_anchor"


EXACT_CASES = [
    {
        "requirement_id": "ecma262-22.2.2.4-0012",
        "family": "end_anchor_grammar_routes_to_assertion_matcher",
        "pattern": "$",
        "flags": "",
        "input": "abc",
        ...
        "expected_match_text": "",
        "expected_behavior": "end_anchor_zero_width_match_at_input_end",
        "obligation": (
            "Assertion :: $ is a zero-width assertion and can return c(x) "
            "without consuming input at the end of the input"
        ),
    },
    {
        "requirement_id": "ecma262-22.2.2.4-0019",
        "family": "end_anchor_accepts_input_end",
        "pattern": "a$",
        "flags": "",
        "input": "ba",
        "expected_search_result": "true",
        ...
        "expected_behavior": "end_anchor_condition_input_end_success",
        "obligation": (
            "If e = InputLength, the $ assertion succeeds and invokes the "
            "continuation with the same match state"
        ),
    },
    {
        "requirement_id": "ecma262-22.2.2.4-0021",
        "family": "end_anchor_rejects_before_trailing_character",
        "pattern": "a$",
        "flags": "",
        "input": "ab",
        ...
        "expected_end_index": "",
        "expected_match_text": "",
        "expected_behavior": "end_anchor_condition_failure_blocks_search_loop",
        "obligation": (
            "When e is not InputLength and multiline does not permit the "
            "position, the $ assertion returns failure; the public search loop "
            "must not turn the earlier atom match into an unanchored match"
        ),
    },
    {
        "requirement_id": "ecma262-22.2.2.4-0019",
        "family": "end_anchor_multiline_accepts_before_lf",
        "pattern": "a$",
        "flags": "m",
        "input": "a\\nx",
        "expected_search_result": "true",
        ...
        "expected_match_text": "a",
        "expected_behavior": "end_anchor_multiline_line_terminator_success",
        "obligation": (
            "If rer.[[Multiline]] is true and Input[e] is a line terminator, "
            "the $ assertion succeeds at that position"
        ),
    },
    {
        "requirement_id": "ecma262-22.2.2.4-0019",
        "family": "end_anchor_multiline_accepts_before_cr",
        "pattern": "a$",
        "flags": "m",
        "input": "a\\rx",
        "expected_search_result": "true",
    ...
    {
        "requirement_id": "ecma262-22.2.2.4-0021",
        "family": "end_anchor_without_multiline_rejects_before_lf",
        "pattern": "a$",
        "flags": "",
        "input": "a\\n",
        "expected_search_result": "false",
        ...
        "expected_end_index": "",
        "expected_match_text": "",
        "expected_behavior": "end_anchor_condition_failure_without_multiline",
        "obligation": (
            "Without rer.[[Multiline]], a line terminator at e does not make "
            "$ succeed unless e is InputLength"
        ),
    },
]
...
def validate_requirement_row(row: dict[str, str]) -> None:
    ...
    validate_expected_fields(row, expected, context="end-anchor row")
    require_coverage_area(
        row,
        "regexp_exec_and_captures",
        context="end-anchor row",
    )
...
def plan_row(
    ...
    exact_case_id = (
        f"match-engine-end-anchor-exact:{requirement_id}:{safe_id(case['family'])}"
    )
    if source_failure_count > 0:
        plan_reason = (
            "JSON Schema failure inventory currently exposes end-anchor as a "
            "matcher blocker; exact ECMA-262 assertion cases must go green "
            "before any corpus-driven implementation credit"
        )
    else:
        plan_reason = (
            "JSON Schema failure inventory no longer exposes end-anchor as a "
            "current blocker; the exact ECMA-262 assertion cases remain in the "
            "coverage system as regression evidence"
        )
    return {
        "plan_id": f"match-engine-end-anchor-exact-plan:{requirement_id}:{safe_id(case['family'])}",
        **copy_requirement_metadata(requirement, include_local_id=False),
        ...
        "coverage_credit": "none_match_engine_end_anchor_exact_planned",
...
def main() -> None:
    ...
    summary_lines = [
        ...
        f"match_engine_end_anchor_exact_plan_rows\t{len(rows)}\n",
