...
DETAIL_NAME = "ecma262-regexp-match-engine-capture-exact-plan.tsv"
SUMMARY_NAME = "ecma262-regexp-match-engine-capture-exact-plan.summary"
TARGET_TEST_ARTIFACT = "test/test_ecma262_match_engine_capture_exact_plan.ml"

OBSERVATIONS_BY_SUFFIX = {
    33: "capture_group_atom",
    34: "capture_subpattern_matcher",
    35: "capture_paren_index",
    36: "capture_matcher_closure",
    37: "capture_match_state_parameter",
    38: "capture_continuation_parameter",
    39: "capture_nested_continuation",
    40: "capture_nested_match_state_parameter",
    41: "capture_copy",
    42: "capture_input_preserved",
    43: "capture_start_index",
    44: "capture_end_index",
    45: "capture_forward_branch",
    46: "capture_forward_order",
    47: "capture_forward_range",
    48: "capture_backward_branch",
    49: "capture_backward_direction",
    50: "capture_backward_order",
    51: "capture_backward_range",
    52: "capture_slot_write",
    53: "capture_result_state",
    54: "capture_outer_continuation",
    55: "capture_submatcher_invocation",
}
...
def validate_source_row(row: dict[str, str]) -> None:
    ...
    expected = {
        ...
        "atom_subfamily": "capturing_group_atom",
        "atom_semantic_route": "capture_range_match_state_model",
        "expected_behavior": "requires_capture_model",
        "coverage_credit": "none_match_engine_atoms_exact_deferred",
        "plan_state": "deferred_requires_capture_model",
        "observability_status": "requires_capture_model",
    }
    validate_expected_fields(row, expected, context="capture exact source row")
    number = suffix_number(requirement_id)
    if number not in OBSERVATIONS_BY_SUFFIX:
        raise SystemExit(
            f"capture exact source row {requirement_id} has unsupported suffix"
        )
    if not Path(row["source_file"]).is_file():
        raise SystemExit(
            f"capture exact source row {requirement_id} source is missing: "
            f"{row['source_file']}"
        )
...
def selected_source_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    return select_expected_source_rows(
        rows,
        include_row=lambda row: row["plan_state"] == "deferred_requires_capture_model",
        expected_ids=expected_ids,
        validate_row=validate_source_row,
        duplicate_message=lambda requirement_id: (
            f"duplicate capture source row for {requirement_id}"
        ),
        missing_prefix="capture exact source rows missing from atom plan: ",
        extra_prefix="capture exact source rows outside expected CompileAtom range: ",
    )
...
def plan_row(row: dict[str, str]) -> dict[str, str]:
    ...
    exact_case_id = (
        f"match-engine-capture-exact:{requirement_id}:{safe_id(observation)}"
    )
    return {
        "plan_id": f"match-engine-capture-exact-plan:{requirement_id}",
        ...
        **copy_requirement_metadata(row, include_local_id=True),
        ...
        "observability_reason": (
            "Ecma_regex_core exposes a test-only capture matcher model "
            "observation without adding public Ecma_regex API surface"
        ),
        "next_action": "materialize_match_engine_capture_exact_case",
        "plan_reason": (
            "capture exact case upgrades a previously deferred CompileAtom "
            "capturing-group row into executable internal matcher-model "
            "evidence; ledger credit is assigned only after the exactness audit "
            "consumes this plan"
        ),
    }
...
def main() -> None:
    ...
    route_counts = Counter(row["capture_semantic_route"] for row in rows)
    summary_lines = [
        "ecma262_snapshot\t2026\n",
        f"input_atoms_plan\t{atoms_plan}\n",
        f"match_engine_capture_exact_plan_rows\t{len(rows)}\n",
        f"planned_executable_rows\t{state_counts.get('planned_not_executable', 0)}\n",
        f"capture_route_capture_range_match_state_model\t{route_counts.get('capture_range_match_state_model', 0)}\n",
