...
DETAIL_NAME = (
    "ecma262-regexp-match-engine-unicode-sets-escape-string-exact-plan.tsv"
)
SUMMARY_NAME = (
    "ecma262-regexp-match-engine-unicode-sets-escape-string-exact-plan.summary"
)
TARGET_TEST_ARTIFACT = (
    "test/test_ecma262_match_engine_unicode_sets_escape_string_exact_plan.ml"
)
RGI_FAMILY = "\U0001F468\u200d\U0001F469\u200d\U0001F467\u200d\U0001F466"

OBSERVATIONS_BY_SUFFIX = {
    78: "unicode_sets_matcher_list_initialized",
    79: "unicode_sets_multi_char_elements_descending_iteration",
    80: "unicode_sets_last_code_point_charset",
    81: "unicode_sets_last_code_point_matcher",
    82: "unicode_sets_prefix_code_point_iteration",
    83: "unicode_sets_prefix_code_point_charset",
    84: "unicode_sets_prefix_code_point_matcher",
    85: "unicode_sets_match_sequence_built",
    86: "unicode_sets_multi_matcher_appended",
    87: "unicode_sets_singles_charset_built",
    88: "unicode_sets_singles_matcher_appended",
    89: "unicode_sets_empty_sequence_checked",
    90: "unicode_sets_empty_matcher_appended",
    91: "unicode_sets_last_matcher_selected",
    92: "unicode_sets_match_two_alternatives_fold",
    93: "unicode_sets_final_matcher_return",
}
...
def validate_source_row(row: dict[str, str]) -> None:
    ...
    expected = {
        ...
        "atom_subfamily": "character_class_escape_unicode_sets_string_elements",
    ...
    validate_expected_fields(
        row,
        expected,
        context="UnicodeSets escape string exact source row",
    )
    number = suffix_number(requirement_id)
    if number not in OBSERVATIONS_BY_SUFFIX:
        raise SystemExit(
            f"UnicodeSets escape string exact source row {requirement_id} has "
            "unsupported suffix"
        )
    if not Path(row["source_file"]).is_file():
        raise SystemExit(
            f"UnicodeSets escape string exact source row {requirement_id} "
            f"source is missing: {row['source_file']}"
        )
...
def selected_source_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    return select_expected_source_rows(
        ...
        duplicate_message=lambda requirement_id: (
            "duplicate UnicodeSets escape string source row for "
            f"{requirement_id}"
        ),
        missing_prefix=(
            "UnicodeSets escape string exact source rows missing from atom plan: "
        ),
        extra_prefix=(
            "UnicodeSets escape string exact source rows outside expected "
            "CompileAtom range: "
        ),
    )
...
def plan_row(row: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        **copy_requirement_metadata(row, include_local_id=True),
        ...
        "flags": "v",
        "input_text": RGI_FAMILY,
        "expected_exec_text": RGI_FAMILY,
        ...
        "observability_reason": (
            ...
            "matcher-model observation for the CharacterClassEscape route "
        ...
        "plan_reason": (
            "UnicodeSets escape string exact case upgrades previously deferred "
            "CompileAtom CharacterClassEscape rows into executable internal "
...
def main() -> None:
    ...
    summary_lines = [
        ...
        f"match_engine_unicode_sets_escape_string_exact_plan_rows\t{len(rows)}\n",
