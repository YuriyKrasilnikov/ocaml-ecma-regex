let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir
        "cache/ecma262-regexp-match-engine-result-exact-plan.tsv"
    in
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-match-engine-result-exact-plan.tsv is missing; \
           run tools/build_ecma262_regexp_match_engine_result_exact_plan.py"
      else climb parent
  in
  climb cwd
...
let plan_rows () =
  read_tsv [ "cache"; "ecma262-regexp-match-engine-result-exact-plan.tsv" ]
...
let planned_rows rows =
  List.filter
    (fun row ->
      ...
      && field "coverage_credit" row = "none_match_engine_result_exact_planned")
    rows

let test_exact_plan_manifest () =
  let rows = plan_rows () in
  Alcotest.(check int) "result exact plan rows" 1 (List.length rows);
  let planned = planned_rows rows in
  Alcotest.(check int) "planned executable rows" 1 (List.length planned);
  let state_counts = count_by "plan_state" rows in
  check_count state_counts "planned_not_executable" 1;
  let credit_counts = count_by "coverage_credit" rows in
  check_count credit_counts "none_match_engine_result_exact_planned" 1;
  let behavior_counts = count_by "expected_behavior" rows in
  check_count behavior_counts "exec_left_priority_match" 1;
  let layer_counts = count_by "executable_layer" rows in
  check_count layer_counts "match_engine" 1;
  let mapping_counts = count_by "mapping_family" rows in
  check_count mapping_counts "match_engine_alternation" 1;
  let observability_counts = count_by "observability_status" rows in
  check_count observability_counts "exec_result_observable" 1;
  List.iter
    (fun row ->
      if
        not
...
let test_exact_plan_result_cases () =
  plan_rows () |> planned_rows |> List.iter check_result_case

let () =
  Alcotest.run "ecma262-match-engine-result-exact-plan"
    [
      ( "manifest",
        [
          Alcotest.test_case "match-engine result exact plan invariants" `Quick
            test_exact_plan_manifest;
        ] );
      ( "exec",
        [
          Alcotest.test_case "match-engine result exact planned cases" `Quick
            test_exact_plan_result_cases;
        ] );
    ]
