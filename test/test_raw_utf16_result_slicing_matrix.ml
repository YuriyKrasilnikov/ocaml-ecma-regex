...
let check_slice_result name input_units expected actual =
  ...
  Alcotest.(check int) (name ^ ": start_index") expected_start actual.Ecma_regex.js_start_index;
  Alcotest.(check int) (name ^ ": end_index") expected_end actual.Ecma_regex.js_end_index;
  ...
  Alcotest.(check (list int))
    ...
    expected.matched_units
    actual_matched_units;
  let actual_prefix = take actual.Ecma_regex.js_start_index input_units in
  let actual_match_from_input =
    slice actual.Ecma_regex.js_start_index actual.Ecma_regex.js_end_index input_units
  in
  let actual_suffix = drop actual.Ecma_regex.js_end_index input_units in
  Alcotest.(check (list int)) (name ^ ": prefix slice") expected.prefix_units actual_prefix;
  Alcotest.(check (list int))
    (name ^ ": matched input slice")
    actual_match_from_input
    actual_matched_units;
  Alcotest.(check (list int)) (name ^ ": suffix slice") expected.suffix_units actual_suffix;
  Alcotest.(check (list int))
...
let run_instance_case
  ...
  (match Ecma_regex.exec_instance_js instance input with
   ...
   | Some actual -> check_slice_result instance_name input_units instance_expected actual);
  Alcotest.(check int)
...
let exec_cases =
  [ exec_case
      ~flags:"u"
      "exec /u braced literal returns whole surrogate-pair slice"
      "\\u{1F600}"
  ...
  ; exec_case
      ...
      (expected [ letter_a ] grinning_face [ letter_b ])
  ; exec_case
      "exec non-Unicode high surrogate returns high-only slice"
      "\\uD83D"
      (expected [ letter_a ] [ high ] [ low; letter_b ])
  ; exec_case
      "exec non-Unicode low surrogate returns low-only slice"
      "\\uDE00"
      (expected [ letter_a; high ] [ low ] [ letter_b ])
  ; exec_case
      ~flags:"u"
  ...
  ; exec_case
      ~flags:"u"
      "exec /u word boundary after pair returns empty slice before A"
      "\\b"
      (expected grinning_face [] [ letter_a ])
  ; exec_case
      ~flags:"u"
      "exec /u start anchor returns empty prefix slice"
      "^"
      (expected [] [] grinning_then_a)
  ; exec_case
      ~flags:"u"
      "exec /u end anchor returns empty suffix slice"
      "$"
      (expected [ letter_a; high; low ] [] [])
  ; exec_case
      ~flags:"iu"
      "exec /iu word character returns folded long-s slice"
      "\\w"
...
let instance_cases =
  [ instance_case
      ~flags:"uy"
      "instance /uy dot at pair start returns whole pair slice"
      ~last_index:1
...
let iterator_cases =
  [ iterator_case
      ~flags:"gu"
      "iterator /gu empty slicing over pair and BMP"
      ""
      grinning_then_a
  ...
  ; iterator_case
      ~flags:"g"
      "iterator /g empty slicing one code unit at a time"
      ""
      grinning_then_a
  ...
  ; iterator_case
      ~flags:"gu"
      "iterator /gu empty slicing from explicit low index"
      ~initial_last_index:1
      ""
      grinning_then_a
      [ step (expected [ high ] [] [ low; letter_a ]) 2
      ; step (expected grinning_face [] [ letter_a ]) 3
  ...
  ; iterator_case
      ~flags:"gu"
      "iterator /gu literal pair slices repeated matches"
      "\\u{1F600}"
      grinning_a_grinning
...
let iterator_tests =
  List.map
    (fun test_case ->
       Alcotest.test_case test_case.iterator_name `Quick (run_iterator_case test_case))
    iterator_cases
;;

let () =
  Alcotest.run
    "raw-utf16-result-slicing-matrix"
