...
let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir "cache/ecma262-regexp-exec-result-exec-exact-plan.tsv"
    in
...
let bool_text = function true -> "true" | false -> "false"
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "exec-result exec exact plan rows" 21 (List.length rows);
  Alcotest.(check int)
    "planned executable rows" 21
  ...
  check_count observability_counts "internal_exec_result_exec_model_observable"
    21;
  let target_counts = count_by "target_test_artifact" rows in
  check_count target_counts "test/test_ecma262_exec_result_exec_exact_plan.ml"
    21;
  List.iter
    (fun row ->
      if
      ...
      Alcotest.(check string)
        "next action" "materialize_exec_result_exec_exact_case"
...
let check_bool_model_field name actual = Alcotest.(check bool) name true actual
...
let check_test_result row observation =
  ...
  | "false" ->
      Alcotest.(check string)
        ...
        (bool_text observation.Core.test_result)
  | value ->
      Alcotest.failf "%s: invalid expected_test_result %S" (field "plan_id" row)
        value
...
let check_exec_case row =
  ...
  | Error msg ->
      Alcotest.failf
        "exec-result exec exact case failed to compile: plan=%s requirement=%s \
