...
let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir
        "cache/ecma262-regexp-match-engine-concatenation-exact-plan.tsv"
    in
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-match-engine-concatenation-exact-plan.tsv is \
           missing; run \
  ...
  List.filter
    (fun row ->
      ...
      && field "coverage_credit" row
         = "none_match_engine_concatenation_exact_planned")
    rows
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "MatchSequence exact plan rows" 14 (List.length rows);
  let planned = planned_rows rows in
  Alcotest.(check int) "planned executable rows" 14 (List.length planned);
  let state_counts = count_by "plan_state" rows in
  check_count state_counts "planned_not_executable" 14;
  let credit_counts = count_by "coverage_credit" rows in
  check_count credit_counts "none_match_engine_concatenation_exact_planned" 14;
  let behavior_counts = count_by "expected_behavior" rows in
  check_count behavior_counts "match_sequence_exact_plan_observable" 14;
  let search_counts = count_by "expected_search_result" rows in
  check_count search_counts "true" 14;
  let observation_counts = count_by "expected_observation" rows in
  List.iter
    (fun name -> check_count observation_counts name 1)
    expected_observations;
  let route_counts = count_by "case_route" rows in
  check_count route_counts "forward_sequence_success" 6;
  check_count route_counts "backward_lookbehind_sequence_success" 8;
  ...
  check_count mapping_counts "match_engine_concatenation" 14;
  let subfamily_counts = count_by "match_sequence_subfamily" rows in
  check_count subfamily_counts "match_sequence_operation" 14;
  let semantic_route_counts = count_by "match_sequence_route" rows in
  check_count semantic_route_counts "matcher_continuation_runtime_semantics" 14;
  let observability_counts = count_by "observability_status" rows in
  check_count observability_counts "match_sequence_model_observable" 14;
  List.iter
    (fun row ->
      if
        not
...
let bool_of_field row name =
  ...
  | "false" -> false
  | value ->
      Alcotest.failf "%s: invalid bool %s=%S" (field "plan_id" row) name value

let observation_value observation = function
  | "match_sequence_operation" ->
      observation.Core.match_sequence_operation_observed
  | "match_sequence_forward_branch" ->
      observation.Core.match_sequence_forward_branch_observed
  | "match_sequence_forward_closure" ->
      observation.Core.match_sequence_forward_closure_observed
  | "match_sequence_forward_match_state_parameter" ->
      observation.Core.match_sequence_forward_match_state_parameter_observed
  | "match_sequence_forward_continuation_parameter" ->
      observation.Core.match_sequence_forward_continuation_parameter_observed
  | "match_sequence_forward_nested_match_state_parameter" ->
      observation
        .Core.match_sequence_forward_nested_match_state_parameter_observed
  | "match_sequence_backward_branch" ->
      observation.Core.match_sequence_backward_branch_observed
  | "match_sequence_backward_closure" ->
      observation.Core.match_sequence_backward_closure_observed
  | "match_sequence_backward_match_state_parameter" ->
      observation.Core.match_sequence_backward_match_state_parameter_observed
  | "match_sequence_backward_continuation_parameter" ->
      observation.Core.match_sequence_backward_continuation_parameter_observed
  | "match_sequence_backward_nested_continuation" ->
      observation.Core.match_sequence_backward_nested_continuation_observed
  | "match_sequence_backward_nested_match_state_parameter" ->
      observation
        .Core.match_sequence_backward_nested_match_state_parameter_observed
  | "match_sequence_backward_first_matcher_return" ->
      observation.Core.match_sequence_backward_first_matcher_return_observed
  | "match_sequence_backward_second_matcher_return" ->
      observation.Core.match_sequence_backward_second_matcher_return_observed
  | name -> Alcotest.failf "unknown observation %S" name

let check_match_sequence_case row =
  let flags = flags_for row in
  match Core.compile ~flags (field "pattern" row) with
  | Error msg ->
      Alcotest.failf
        "MatchSequence exact case failed to compile: plan=%s requirement=%s \
  ...
  | Ok regexp ->
      ...
      let observation =
        Core.inspect_match_sequence_model regexp (field "input_text" row)
      in
      Alcotest.(check bool)
...
let test_exact_plan_match_sequence_cases () =
  plan_rows () |> planned_rows |> List.iter check_match_sequence_case

let () =
  Alcotest.run "ecma262-match-engine-concatenation-exact-plan"
    [
      ( "manifest",
        [
          Alcotest.test_case "MatchSequence exact plan invariants" `Quick
            test_exact_plan_manifest;
        ] );
      ( "model",
        [
          Alcotest.test_case "MatchSequence exact planned cases" `Quick
            test_exact_plan_match_sequence_cases;
        ] );
    ]
