let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir "cache/ecma262-regexp-compile-parser-candidate-map.tsv"
    in
...
let split_csv value =
  List.filter (( <> ) "") (String.split_on_char ',' value)

let has_csv value expected =
  List.exists (( = ) expected) (split_csv value)
...
let test_post_credit_candidate_map_manifest () =
  ...
  in
  Alcotest.(check int) "rows with compile candidates" 0
    (List.length rows_with_candidates);
  Alcotest.(check int) "rows without compile candidates" 0
  ...
  List.iter
    (fun row ->
       ...
       Alcotest.(check string) "release gate" "blocking" (field "release_gate" row);
       Alcotest.(check string)
         "mapping state"
         "open_exact_case_selection"
         (field "mapping_state" row);
       Alcotest.(check string) "exact test case id" "" (field "exact_test_case_id" row);
       Alcotest.(check string) "exact test source" "" (field "exact_test_source" row);
       Alcotest.(check string) "expected behavior" "" (field "expected_behavior" row);
       Alcotest.(check string) "exactness case id" "" (field "exactness_case_id" row);
       Alcotest.(check string)
       ...
       if
         ...
         && not (has_csv (field "candidate_selector_tags" row) "negative_syntax_needed")
       then Alcotest.failf "%s: negative/local row lacks negative selector"
           (field "requirement_id" row))
    rows;
  List.iter
    (fun row ->
       if not (String.starts_with ~prefix:"test262-compile:" (field "case_id" row))
       then Alcotest.failf "feature row has invalid case_id %S" (field "case_id" row);
       if not (test262_source_exists (field "source_path" row)) then
         Alcotest.failf "%s: missing test262 source %s"
           (field "case_id" row)
...
let () =
  Alcotest.run "ecma262-compile-parser-candidate-map" [
    ("manifest", [
      Alcotest.test_case "post-credit compile/parser candidate map invariants"
        `Quick
