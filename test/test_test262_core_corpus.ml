let repo_root () =
  ...
  let rec climb dir =
    ...
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/test262-regexp-candidate-audit.tsv is missing; run \
           tools/audit_test262_regexp_candidates.py"
      else climb parent
  in
  climb cwd
...
let audit_rows () = read_tsv [ "cache"; "test262-regexp-candidate-audit.tsv" ]
...
let list_mem_string needle haystack = List.exists (String.equal needle) haystack

let test_promoted_core_manifest () =
  let rows = audit_rows () in
  let promoted =
    rows |> List.filter (fun row -> field "action" row = "promote_core_corpus")
  in
  ...
  List.iter
    (fun row ->
      Alcotest.(check string)
        "promoted class" "core_regexp_semantics"
...
let test_core_compile_case_manifest () =
  ...
  let no_flag_cases = List.filter (fun row -> field "flags" row = "") cases in
  let flagged_cases = List.filter (fun row -> field "flags" row <> "") cases in
  Alcotest.(check int) "literal compile cases" 729 (List.length cases);
  Alcotest.(check int)
    "literal compile cases without flags" 196
    (List.length no_flag_cases);
  Alcotest.(check int)
    "literal compile cases with flags" 533
    (List.length flagged_cases);
  List.iter
    (fun row ->
...
let test_compile_no_flag_literals () =
  let cases =
    compile_cases () |> List.filter (fun row -> field "flags" row = "")
  in
  List.iteri
    (fun index row ->
      ...
      | Error msg ->
          Alcotest.failf
            "test262 accepted literal should compile (%d/%d): %s:%s raw=%S \
             pattern=%S error=%s"
            (index + 1) (List.length cases) (field "source_path" row)
            (field "line" row) (field "raw" row) pattern msg)
    cases
  ...
  List.iteri
    (fun index row ->
      ...
      let flags =
        ...
        | Error msg ->
            Alcotest.failf
              "test262 accepted literal has flags that should parse (%d/%d): \
               %s:%s raw=%S flags=%S error=%s"
              (index + 1) (List.length cases) (field "source_path" row)
              (field "line" row) (field "raw" row) source_flags msg
      in
      let pattern = field "pattern" row in
      ...
      | Error msg ->
          Alcotest.failf
            "test262 accepted flagged literal should compile (%d/%d): %s:%s \
             raw=%S pattern=%S flags=%S error=%s"
            (index + 1) (List.length cases) (field "source_path" row)
            (field "line" row) (field "raw" row) pattern source_flags msg)
    cases
