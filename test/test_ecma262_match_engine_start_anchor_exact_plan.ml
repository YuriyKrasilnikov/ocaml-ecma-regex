...
let plan_rows () =
  read_tsv [
    "cache";
    "ecma262-regexp-match-engine-start-anchor-exact-plan.tsv";
  ]
...
let test_exact_plan_manifest () =
  ...
  check_count behavior_counts
    "start_anchor_zero_width_match_at_input_start" 1;
  check_count behavior_counts
    "start_anchor_condition_input_start_success" 1;
  check_count behavior_counts
    "start_anchor_condition_failure_blocks_search_loop" 1;
  check_count behavior_counts
    "start_anchor_multiline_line_terminator_success" 2;
  check_count behavior_counts
    "start_anchor_condition_failure_without_multiline" 1;
...
let test_exact_plan_start_anchor_cases () =
  plan_rows ()
  |> planned_rows
  |> List.iter check_start_anchor_case
