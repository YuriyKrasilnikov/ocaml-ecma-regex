...
let test_executable_plan_manifest () =
  ...
  List.iter
    (fun row ->
       ...
       then Alcotest.failf "%s: executable gate must not credit coverage"
           (field "plan_id" row))
    rows;
  List.iter
    (fun row ->
       ...
       Alcotest.(check string)
         "coverage credit"
         "none_reused_candidate_exact_planned"
...
let () =
  Alcotest.run "ecma262-reused-candidate-exact-compile-parser" [
    ("manifest", [
      Alcotest.test_case "reused candidate exact executable plan invariants" `Quick
        test_executable_plan_manifest;
    ]);
    ("compile", [
      Alcotest.test_case "reused candidate exact planned cases compile" `Quick
        test_reused_candidate_exact_compile_parser;
    ]);
  ]
