let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir "cache/ecma262-regexp-reused-candidate-exact-plan.tsv"
    in
...
let planned_rows rows =
  List.filter
    (fun row -> field "plan_state" row = "planned_not_executable")
    rows

let manual_rows rows =
  List.filter
    (fun row -> field "plan_state" row = "manual_spec_review_required")
    rows

let test_executable_plan_manifest () =
  let rows = plan_rows () in
  let planned = planned_rows rows in
  let manual = manual_rows rows in
  Alcotest.(check int) "exact plan rows" 226 (List.length rows);
  Alcotest.(check int) "planned executable rows" 224 (List.length planned);
  Alcotest.(check int) "manual review rows" 2 (List.length manual);
  check_unique_nonempty "exact_case_id" planned;
  ...
  List.iter
    (fun row ->
      ...
      then
        Alcotest.failf "%s: exact_case_id must use reused-exact prefix"
          (field "plan_id" row);
      Alcotest.(check string)
        "expected behavior" "compile_ok"
      ...
      Alcotest.(check string)
        "coverage credit" "none_reused_candidate_exact_planned"
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        ...
        (field "target_test_artifact" row);
      Alcotest.(check string)
        "coverage credit" "none_manual_review_required"
        (field "coverage_credit" row))
    manual

let compile_planned_case index total row =
  ...
  let compile_result =
    if flags_source = "" then Ecma_regex.compile pattern
      ...
      | Error msg ->
          Alcotest.failf
            "reused exact planned case has flags that should parse (%d/%d): \
             requirement=%s exact_case=%s flags=%S error=%s"
            (index + 1) total
  ...
  | Error msg ->
      Alcotest.failf
        "reused exact planned case should compile (%d/%d): requirement=%s \
         exact_case=%s selected_case=%s pattern=%S flags=%S error=%s"
        (index + 1) total
        (field "requirement_id" row)
        (field "exact_case_id" row)
        (field "selected_case_id" row)
        pattern flags_source msg

let test_reused_candidate_exact_compile_parser () =
  let planned = plan_rows () |> planned_rows in
  List.iteri
    (fun index row -> compile_planned_case index (List.length planned) row)
    planned

let () =
  Alcotest.run "ecma262-reused-candidate-exact-compile-parser"
    [
      ( "manifest",
        [
          Alcotest.test_case "reused candidate exact executable plan invariants"
            `Quick test_executable_plan_manifest;
        ] );
      ( "compile",
        [
          Alcotest.test_case "reused candidate exact planned cases compile"
            `Quick test_reused_candidate_exact_compile_parser;
        ] );
    ]
