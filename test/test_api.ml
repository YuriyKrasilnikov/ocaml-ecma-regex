...
let test_compile_accepts_control_letter_escape () =
  ...
  | Ok flags -> (
      ...
      | Ok _ -> ()
      | Error msg ->
          Alcotest.failf "control-letter escape unexpectedly failed: %s" msg)
...
let test_compile_accepts_negative_lookahead () =
  ...
  | Ok flags -> (
      ...
      | Ok _ -> ()
      | Error msg ->
          Alcotest.failf "negative lookahead unexpectedly failed: %s" msg)
  ...
  | Ok flags ->
      List.iter
        (fun pattern ->
          ...
          | Ok _ ->
              Alcotest.failf
                "unicode quantified lookahead %S unexpectedly succeeded" pattern)
        [ "(?=.)*"; "(?!.){1,2}?" ]
...
let test_compile_rejects_invalid_unicode_decimal_escape () =
  ...
  | Ok flags -> (
      ...
      | Error _ -> ()
      | Ok _ ->
          Alcotest.fail "invalid unicode decimal escape unexpectedly succeeded")
...
let test_compile_accepts_valid_unicode_decimal_escape () =
  ...
  | Ok flags -> (
      ...
      | Ok _ -> ()
      | Error msg ->
          Alcotest.failf "valid unicode decimal escape unexpectedly failed: %s"
            msg)
...
let test_compile_accepts_modifiers_group () =
  List.iter
    (fun pattern ->
      ...
      | Ok _ -> ()
      | Error msg ->
          Alcotest.failf "modifiers group %S unexpectedly failed: %s" pattern
            msg)
    [ "(?i:a)"; "(?m:a)"; "(?s:a)"; "(?i-m:a)" ]
...
let check_match_result name expected_start expected_end expected_text
  ...
  Alcotest.(check int) (name ^ " start_index") expected_start result.start_index;
  Alcotest.(check int) (name ^ " end_index") expected_end result.end_index;
  Alcotest.(check string)
    (name ^ " matched_text") expected_text result.matched_text
...
let check_js_match_result name expected_start expected_end expected_units
  ...
  Alcotest.(check int)
    (name ^ " start_index") expected_start result.js_start_index;
  Alcotest.(check int) (name ^ " end_index") expected_end result.js_end_index;
  Alcotest.(check (list int))
    (name ^ " matched UTF-16 units")
    expected_units
    (Ecma_regex.js_string_to_utf16_code_units result.js_matched_text)

let check_js_capture name expected_index expected_start expected_end
    expected_units (capture : Ecma_regex.js_capture) =
  Alcotest.(check int)
    (name ^ " capture index") expected_index capture.js_capture_index;
  Alcotest.(check (option int))
    (name ^ " capture start") expected_start capture.js_capture_start_index;
  Alcotest.(check (option int))
    (name ^ " capture end") expected_end capture.js_capture_end_index;
  let actual_units =
...
let check_js_named_capture name expected_name expected_capture
  ...
  Alcotest.(check string)
    (name ^ " capture name") expected_name named_capture.js_named_capture_name;
...
let test_exec_reports_utf16_indices_for_unicode_non_bmp_literal () =
  ...
  List.iter
    (fun ( name,
      ...
      | Some result ->
          Alcotest.(check int)
            (name ^ " start_index") expected_start result.start_index;
          Alcotest.(check int)
            (name ^ " end_index") expected_end result.end_index;
          Alcotest.(check string)
            (name ^ " matched_text") expected_text result.matched_text)
    cases
...
let test_unicode_raw_non_bmp_literal_quantifier_anchors () =
  ...
  List.iter
    (fun (name, input, expected) ->
      Alcotest.(check bool) name expected (Ecma_regex.search regexp input))
    cases
...
let test_unicode_dot_uses_code_point_line_terminators () =
  ...
  Alcotest.(check bool)
    "unicode dot excludes U+2028" true
...
let test_unicode_search_does_not_probe_inside_non_bmp_utf8 () =
  ...
  List.iter
    (fun (name, flag_source, pattern, input, expected) ->
      ...
      | None, None -> ()
      | None, Some result ->
          Alcotest.failf "%s unexpectedly matched %d..%d %S" name
            result.start_index result.end_index result.matched_text
...
let test_sticky_exec_does_not_search_past_initial_index () =
  ...
  Alcotest.(check bool)
    "sticky failure at index 0" true
  ...
  in
  Alcotest.(check bool)
    "unicode sticky does not advance after failure" true
...
let test_global_instance_updates_last_index () =
  ...
  Alcotest.(check int) "initial last_index" 0 (Ecma_regex.last_index instance);
  ...
  Alcotest.(check int)
    "last_index after first match" 2
  ...
  Alcotest.(check int)
    "last_index after second match" 3
    (Ecma_regex.last_index instance);
  Alcotest.(check bool)
    "global failure at tail" true
    (Option.is_none (Ecma_regex.exec_instance instance "baac"));
  Alcotest.(check int)
    "last_index reset after global failure" 0
    (Ecma_regex.last_index instance)
...
let test_unicode_global_instance_uses_utf16_last_index () =
  ...
  Alcotest.(check int)
    "last_index after non-BMP prefix" 3
    (Ecma_regex.last_index instance)
...
let test_sticky_instance_uses_explicit_last_index () =
  ...
  | Some result ->
      ...
      Alcotest.(check string) "matched_text" "a" result.matched_text);
  Alcotest.(check int)
    "last_index after sticky success" 2
    (Ecma_regex.last_index instance);
  Alcotest.(check bool)
    "sticky tail failure" true
    (Option.is_none (Ecma_regex.exec_instance instance "ba"));
  Alcotest.(check int)
    "last_index reset after sticky failure" 0
    (Ecma_regex.last_index instance)
...
let test_instance_out_of_bounds_last_index_resets () =
  ...
  Alcotest.(check bool)
    "out-of-bounds global exec" true
    (Option.is_none (Ecma_regex.exec_instance instance "a"));
  Alcotest.(check int)
    "last_index reset after out-of-bounds" 0
    (Ecma_regex.last_index instance)
...
let test_non_global_instance_does_not_mutate_last_index () =
  ...
  | Some result ->
      ...
      Alcotest.(check string) "matched_text" "a" result.matched_text);
  Alcotest.(check int)
    "non-global last_index unchanged" 2
    (Ecma_regex.last_index instance)
...
let test_unicode_global_iterator_advances_non_bmp_empty_match () =
  ...
  Alcotest.(check int)
    "unicode last_index after non-BMP advance" 2
    (Ecma_regex.last_index instance);
  (match Ecma_regex.next_match iterator with
  ...
  Alcotest.(check int)
    "unicode last_index after ASCII advance" 3
    (Ecma_regex.last_index instance);
  (match Ecma_regex.next_match iterator with
  ...
  Alcotest.(check int)
    "unicode last_index after terminal empty match" 4
    (Ecma_regex.last_index instance);
  Alcotest.(check bool)
    "unicode iterator done after out-of-bounds step" true
    (Option.is_none (Ecma_regex.next_match iterator));
  Alcotest.(check int)
    "unicode last_index reset after exhaustion" 0
    (Ecma_regex.last_index instance)
...
let test_global_sticky_iterator_advances_after_empty_match () =
  ...
  Alcotest.(check int)
    "last_index after first global-sticky empty match" 1
    (Ecma_regex.last_index instance);
  (match Ecma_regex.next_match iterator with
  ...
  Alcotest.(check int)
    "last_index after second global-sticky empty match" 2
    (Ecma_regex.last_index instance);
  (match Ecma_regex.next_match iterator with
  ...
  Alcotest.(check int)
    "last_index after terminal global-sticky empty match" 3
    (Ecma_regex.last_index instance);
  Alcotest.(check bool)
    "global-sticky iterator done" true
    (Option.is_none (Ecma_regex.next_match iterator));
  Alcotest.(check int)
    "global-sticky last_index reset after exhaustion" 0
    (Ecma_regex.last_index instance)
...
let test_non_global_iterator_is_done_after_first_match () =
  ...
  Alcotest.(check int)
    "non-global iterator keeps last_index" 2
    (Ecma_regex.last_index instance);
  Alcotest.(check bool)
    "non-global iterator is done after first match" true
    (Option.is_none (Ecma_regex.next_match iterator));
  Alcotest.(check int)
    "done non-global iterator keeps last_index" 2
    (Ecma_regex.last_index instance)
...
let test_sticky_non_global_iterator_is_done_after_first_match () =
  ...
  Alcotest.(check int)
    "sticky non-global last_index after empty match" 0
    (Ecma_regex.last_index instance);
  Alcotest.(check bool)
    "sticky non-global iterator done" true
    (Option.is_none (Ecma_regex.next_match iterator));
  Alcotest.(check int)
    "done sticky non-global last_index unchanged" 0
    (Ecma_regex.last_index instance)
...
let test_js_string_rejects_invalid_utf16_units () =
  ...
  List.iter
    (fun units ->
      match Ecma_regex.js_string_of_utf16_code_units units with
      | Error _ -> ()
      | Ok _ -> Alcotest.failf "invalid UTF-16 units unexpectedly accepted")
    cases
...
let test_js_string_lone_surrogate_literals_match_code_units () =
  ...
  List.iter
    (fun (name, flag_source, pattern) ->
      ...
      | Some result -> check_js_match_result name 0 1 [ high_surrogate ] result)
    cases
...
let test_js_string_unicode_surrogate_pair_matches_code_point () =
  ...
  | Some result ->
      check_js_match_result "surrogate pair" 0 2 [ high; low ] result
...
let test_js_string_unicode_empty_iterator_advances_over_surrogate_pair () =
  ...
  | Some result ->
      check_js_match_result "raw UTF-16 empty at start" 0 0 [] result);
  Alcotest.(check int)
    "last_index after surrogate pair advance" 2
    (Ecma_regex.last_index instance)
...
let test_js_string_unicode_empty_iterator_inside_surrogate_pair () =
  ...
  Alcotest.(check int)
    "last_index advances one code unit from low surrogate" 2
    (Ecma_regex.last_index instance)
...
let test_js_match_result_exposes_undefined_capture () =
  ...
  | Some result -> (
      check_js_match_result "optional capture full match" 0 1
        [ Char.code 'b' ]
        result;
      match result.js_captures with
      | [ capture ] ->
...
let test_js_match_result_exposes_named_captures () =
  let input =
    js_string_of_utf16_units_or_fail [ Char.code 'a'; Char.code 'b' ]
  in
  ...
  | Some result -> (
      check_js_match_result "named capture full match" 0 2
        ...
        result;
      Alcotest.(check int)
        "numbered captures include named and unnamed groups" 2
        (List.length result.js_captures);
...
let test_js_match_result_exposes_instance_and_iterator_captures () =
  ...
  let low = 0xDE00 in
  let input =
    js_string_of_utf16_units_or_fail [ high; low; Char.code 'x'; high; low ]
  in
  let regexp = compile_or_fail ~flags "(\\u{1F600})" in
  let instance = Ecma_regex.instance regexp in
...
let test_js_string_unicode_lookbehind_crosses_surrogate_pair () =
  ...
  match Ecma_regex.exec_js regexp input with
  | None ->
      Alcotest.fail "unicode lookbehind after surrogate pair returned None"
  | Some result ->
...
let test_js_string_anchors_use_utf16_boundaries () =
  ...
  | Some result ->
      check_js_match_result "anchors high surrogate" 0 1 [ high ] result
...
let test_js_string_unicode_class_escape_consumes_surrogate_pair () =
  ...
  | Some result ->
      check_js_match_result "unicode \\D surrogate pair" 0 2 [ high; low ]
        result
...
let test_js_string_unicode_simple_class_consumes_surrogate_pair () =
  ...
  | Some result ->
      check_js_match_result "unicode [^a] surrogate pair" 0 2 [ high; low ]
        result
...
let test_js_string_multiline_anchors_use_ecma_line_terminators () =
  ...
  | Some result ->
      check_js_match_result "multiline end before U+2029" 0 1
        [ Char.code 'a' ]
        result
...
let test_search_matches_start_anchor () =
  let regexp = compile_or_fail "^a" in
  Alcotest.(check bool)
    "match at input start" true
    (Ecma_regex.search regexp "ab");
  Alcotest.(check bool)
    "no unanchored later match" false
...
let test_search_matches_end_anchor () =
  let regexp = compile_or_fail "a$" in
  Alcotest.(check bool)
    "match at input end" true
    (Ecma_regex.search regexp "ba");
  Alcotest.(check bool)
    "no unanchored earlier match" false
    (Ecma_regex.search regexp "ab")

let test_search_matches_dot_atom () =
  let regexp = compile_or_fail "." in
  Alcotest.(check bool)
    "dot matches ordinary character" true
    (Ecma_regex.search regexp "a");
  Alcotest.(check bool)
    "dot excludes line feed without s" false
  ...
  | Ok flags ->
      ...
      Alcotest.(check bool)
        "dotAll includes line feed" true
...
let test_search_matches_character_class_atoms () =
  ...
  Alcotest.(check bool) "class non-member" false (Ecma_regex.search regexp "z");
  let inverted = compile_or_fail "[^a]" in
  Alcotest.(check bool)
    "inverted class member" true
    (Ecma_regex.search inverted "b");
  Alcotest.(check bool)
    "inverted class excluded" false
...
let test_search_matches_character_class_escapes () =
  ...
  Alcotest.(check bool) "digit non-member" false (Ecma_regex.search digit "x");
  let non_digit = compile_or_fail "\\D" in
  Alcotest.(check bool)
    "non-digit rejects digit" false
...
let test_compile_accepts_legacy_braced_unicode_text_without_unicode_flag () =
  ...
  | Error msg ->
      Alcotest.failf
        "legacy non-unicode braced unicode text unexpectedly failed: %s" msg
...
let test_compile_accepts_duplicate_named_groups_across_disjunction () =
  ...
  | Error msg ->
      Alcotest.failf
        "duplicate named groups across disjunction unexpectedly failed: %s" msg
...
let test_compile_rejects_named_backreference_without_matching_group () =
  ...
  | Error _ -> ()
  | Ok _ ->
      Alcotest.fail "missing named backreference target unexpectedly succeeded"
    ...
    [
      ( "api",
        [
          ...
          Alcotest.test_case "compile accepts non-unicode identity escape"
            `Quick test_compile_accepts_non_unicode_identity_escape;
          ...
          Alcotest.test_case "compile rejects unicode quantified lookahead"
            `Quick test_compile_rejects_unicode_quantified_lookahead;
          ...
          Alcotest.test_case "compile accepts escaped hyphen in unicode class"
            `Quick test_compile_accepts_escaped_hyphen_in_unicode_class;
          Alcotest.test_case "compile rejects invalid character class range"
            `Quick test_compile_rejects_invalid_character_class_range;
          Alcotest.test_case "compile accepts unicode class syntax char" `Quick
            test_compile_accepts_unicode_class_set_syntax_char;
          Alcotest.test_case "compile rejects unicodeSets class syntax char"
            `Quick test_compile_rejects_unicode_sets_class_syntax_char;
          Alcotest.test_case
            "compile rejects unicodeSets reserved double punctuator" `Quick
            test_compile_rejects_unicode_sets_reserved_double_punctuator;
          Alcotest.test_case
            "compile rejects unicode class escape range endpoint" `Quick
          ...
          Alcotest.test_case "compile rejects invalid unicode decimal escape"
            `Quick test_compile_rejects_invalid_unicode_decimal_escape;
          Alcotest.test_case "compile accepts valid unicode decimal escape"
            `Quick test_compile_accepts_valid_unicode_decimal_escape;
          ...
          Alcotest.test_case "compile accepts supported unicode property escape"
            `Quick test_compile_accepts_supported_unicode_property_escape;
          Alcotest.test_case
            "compile rejects unsupported unicode property escape" `Quick
          ...
          Alcotest.test_case
            "exec reports UTF-16 indices for unicode non-BMP literal" `Quick
            test_exec_reports_utf16_indices_for_unicode_non_bmp_literal;
          Alcotest.test_case "unicode raw non-BMP literal quantifier anchors"
            `Quick test_unicode_raw_non_bmp_literal_quantifier_anchors;
          Alcotest.test_case "unicode dot consumes non-BMP code point" `Quick
            test_unicode_dot_consumes_non_bmp_code_point;
          Alcotest.test_case "unicode dot uses code-point line terminators"
            `Quick test_unicode_dot_uses_code_point_line_terminators;
          Alcotest.test_case
            "unicode search does not probe inside non-BMP UTF-8 sequence" `Quick
            test_unicode_search_does_not_probe_inside_non_bmp_utf8;
          Alcotest.test_case "sticky exec does not search past initial index"
            `Quick test_sticky_exec_does_not_search_past_initial_index;
          Alcotest.test_case "global instance updates last_index" `Quick
            test_global_instance_updates_last_index;
          Alcotest.test_case "unicode global instance uses UTF-16 last_index"
            `Quick test_unicode_global_instance_uses_utf16_last_index;
          ...
          Alcotest.test_case "non-global instance does not mutate last_index"
            `Quick test_non_global_instance_does_not_mutate_last_index;
          Alcotest.test_case "global iterator advances after empty match" `Quick
            test_global_iterator_advances_after_empty_match;
          Alcotest.test_case
            "unicode global iterator advances non-BMP empty match" `Quick
          ...
          Alcotest.test_case "global sticky iterator advances after empty match"
            `Quick test_global_sticky_iterator_advances_after_empty_match;
          Alcotest.test_case "non-global iterator is done after first match"
            `Quick test_non_global_iterator_is_done_after_first_match;
          Alcotest.test_case
            "sticky non-global iterator is done after first match" `Quick
          ...
          Alcotest.test_case
            "JS string lone surrogate literals match code units" `Quick
            test_js_string_lone_surrogate_literals_match_code_units;
          Alcotest.test_case
            "JS string unicode surrogate pair matches code point" `Quick
            test_js_string_unicode_surrogate_pair_matches_code_point;
          Alcotest.test_case "JS string non-unicode can match surrogate half"
            `Quick test_js_string_non_unicode_can_match_surrogate_half;
          Alcotest.test_case
          ...
          Alcotest.test_case
            "JS string unicode empty iterator inside surrogate pair" `Quick
            test_js_string_unicode_empty_iterator_inside_surrogate_pair;
          Alcotest.test_case "JS string non-unicode dot consumes one code unit"
            `Quick test_js_string_non_unicode_dot_consumes_one_code_unit;
          Alcotest.test_case
            "JS string non-unicode class escape consumes one code unit" `Quick
            test_js_string_non_unicode_class_escape_consumes_one_code_unit;
          Alcotest.test_case
            "JS string surrogate pair capture and backreference" `Quick
            test_js_string_surrogate_pair_capture_and_backreference;
          Alcotest.test_case
            "JS match result exposes numbered captures as UTF-16 slices" `Quick
          ...
          Alcotest.test_case "JS match result exposes undefined capture" `Quick
            test_js_match_result_exposes_undefined_capture;
          Alcotest.test_case "JS match result exposes named captures" `Quick
            test_js_match_result_exposes_named_captures;
          Alcotest.test_case
            "JS match result exposes instance and iterator captures" `Quick
            test_js_match_result_exposes_instance_and_iterator_captures;
          Alcotest.test_case
            "JS string unicode lookbehind crosses surrogate pair" `Quick
            test_js_string_unicode_lookbehind_crosses_surrogate_pair;
          Alcotest.test_case "JS string character classes match raw UTF-16"
            `Quick test_js_string_character_classes_match_raw_utf16;
          Alcotest.test_case "JS string anchors use UTF-16 boundaries" `Quick
            test_js_string_anchors_use_utf16_boundaries;
          Alcotest.test_case
            "JS string unicode class escape consumes surrogate pair" `Quick
            test_js_string_unicode_class_escape_consumes_surrogate_pair;
          Alcotest.test_case
            "JS string unicode simple class consumes surrogate pair" `Quick
          ...
          Alcotest.test_case
            "JS string multiline anchors use ECMA line terminators" `Quick
            test_js_string_multiline_anchors_use_ecma_line_terminators;
          Alcotest.test_case
            "JS string word boundary uses Unicode WordCharacters" `Quick
          ...
          Alcotest.test_case
            "compile accepts braced unicode escape with unicode flag" `Quick
            test_compile_accepts_braced_unicode_escape_with_unicode_flag;
          Alcotest.test_case "compile accepts legacy braced unicode text" `Quick
            test_compile_accepts_legacy_braced_unicode_text_without_unicode_flag;
          Alcotest.test_case
            "compile accepts legacy invalid braced unicode text" `Quick
            test_compile_accepts_legacy_invalid_braced_unicode_text;
          Alcotest.test_case
            "compile accepts duplicate named groups across disjunction" `Quick
            test_compile_accepts_duplicate_named_groups_across_disjunction;
          Alcotest.test_case
            "compile rejects duplicate named groups in same alternative" `Quick
          ...
          Alcotest.test_case
            "compile accepts named backreference with matching group" `Quick
            test_compile_accepts_named_backreference_with_matching_group;
          Alcotest.test_case
            "compile rejects named backreference without matching group" `Quick
