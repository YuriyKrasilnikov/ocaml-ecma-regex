...
let source_file row =
  [ "external"; "test262"; field "source_path" row ]
...
let test_manifest () =
  ...
  check_count credit_counts
    "none_test262_literal_lexer_executable_planned" 25;
  ...
  List.iter
    (fun row ->
       ...
       if not (List.mem (field "expected_behavior" row) allowed_behaviors)
       then Alcotest.failf "%s: unsupported expected behavior %s"
           (field "case_id" row)
...
let test_literal_parser_cases () =
  ...
  |> List.iter (fun row ->
      ...
      (match Ecma_regex.regexp_literal_of_string literal_source with
       ...
       | Ok literal ->
         ...
         (match field "expected_compile_result" row with
          | "compile_ok" ->
            (match Ecma_regex.compile ~flags:literal.flags literal.pattern_text with
             | Ok _ -> ()
             | Error error ->
    ...
    | "literal_parse_error" ->
      Alcotest.(check string)
        "parse-error compile result"
        "not_applicable"
        (field "expected_compile_result" row);
      (match Ecma_regex.regexp_literal_of_string literal_source with
       | Error _ -> ()
       | Ok literal ->
         Alcotest.failf "%s: literal parse unexpectedly succeeded as /%s/%s"
           (field "case_id" row)
           literal.pattern_text
           literal.flag_text)
    | result ->
...
let () =
  Alcotest.run "test262-regexp-executable-cases" [
    ...
    ("literal-parser", [
      Alcotest.test_case "literal parser cases" `Quick test_literal_parser_cases;
    ]);
  ]
