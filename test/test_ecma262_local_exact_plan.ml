  ...
  List.iter
    (fun row ->
      ...
      if not (source_exists row) then
        Alcotest.failf "%s: missing source file %s" (field "plan_id" row)
          (field "source_file" row);
      Alcotest.(check string)
        "expected behavior" "compile_ok"
        (field "expected_behavior" row);
      Alcotest.(check string)
        "coverage credit" "none_local_exact_planned"
        (field "coverage_credit" row);
      Alcotest.(check string)
        "plan state" "planned_not_executable" (field "plan_state" row);
      Alcotest.(check string)
        "target artifact" "test/test_ecma262_local_exact_compile_parser.ml"
        (field "target_test_artifact" row);
      Alcotest.(check string)
        "next action" "materialize_local_exact_case" (field "next_action" row))
    rows
