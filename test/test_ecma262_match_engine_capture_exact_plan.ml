...
let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir
        "cache/ecma262-regexp-match-engine-capture-exact-plan.tsv"
    in
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-match-engine-capture-exact-plan.tsv is \
           missing; run \
           tools/build_ecma262_regexp_match_engine_capture_exact_plan.py"
      else climb parent
  in
  climb cwd
...
let planned_rows rows =
  List.filter
    (fun row ->
      ...
      && field "coverage_credit" row = "none_match_engine_capture_exact_planned")
    rows
...
let observation_value observation = function
  ...
  | "capture_forward_branch" -> observation.Core.capture_forward_branch_observed
  ...
  | "capture_backward_order" -> observation.Core.capture_backward_order_observed
  | "capture_backward_range" -> observation.Core.capture_backward_range_observed
...
let check_capture_case row =
  ...
  | Error msg ->
      Alcotest.failf
        "capture exact case failed to compile: plan=%s requirement=%s \
  ...
  | Ok regexp ->
      let observation =
        Core.inspect_capture_model regexp (field "input_text" row)
      in
      Alcotest.(check bool)
