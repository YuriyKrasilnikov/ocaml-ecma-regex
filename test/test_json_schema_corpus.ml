...
let repo_root () =
  ...
  let rec climb dir =
    ...
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "external/json-schema-test-suite is missing; run \
           tools/fetch_json_schema_test_suite.py"
      else climb parent
  in
  climb cwd
    ...
    (fun () ->
      ...
      let rec rows acc =
        ...
        | line -> (
            ...
            | [ "" ] -> rows acc
            | _ ->
                Alcotest.failf "invalid JSON Schema corpus manifest line: %s"
                  line)
        ...
        (fun group ->
          {
            ...
            schema =
              (match member "schema" group with
              | Some s -> s
...
let integer = function `Int _ | `Intlit _ -> true | _ -> false
let number = function `Int _ | `Intlit _ | `Float _ -> true | _ -> false
...
let check_maximum limit value =
  ...
  | Some _, None -> true
  | None, _ ->
      Alcotest.fail "unsupported non-numeric maximum in JSON Schema corpus"

let json_schema_regex_flags = Ecma_regex.flags ~unicode:true ()
...
let validate_pattern pattern value =
  ...
  | Ok re -> (
      match value with `String s -> Ecma_regex.search re s | _ -> true)
...
let rec validate_schema schema value =
  ...
  | `Assoc fields ->
      let compiled_pattern_properties =
        ...
        | Some _ ->
            Alcotest.fail
              "unsupported non-object patternProperties in JSON Schema corpus"
        | None -> []
      in
      let type_valid =
        ...
        | Some _ ->
            Alcotest.fail "unsupported non-string type in JSON Schema corpus"
        | None -> true
      in
      let maximum_valid =
      ...
      let pattern_valid =
        ...
        | Some _ ->
            Alcotest.fail "unsupported non-string pattern in JSON Schema corpus"
        | None -> true
      in
      let pattern_properties_valid =
        match List.assoc_opt "patternProperties" fields with
        | Some (`Assoc _) ->
            validate_pattern_properties_compiled compiled_pattern_properties
              value
        | None -> true
        | Some _ -> assert false
      in
      let additional_properties_valid =
        ...
        | Some additional_schema ->
            validate_additional_properties additional_schema
              compiled_pattern_properties value
        | None -> true
      in
      type_valid && maximum_valid && pattern_valid && pattern_properties_valid
...
let check_schema description schema tests =
  List.iter
    (fun (test_description, data, expected) ->
