let repo_root () =
  ...
  let rec climb dir =
    ...
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-match-engine-exact-plan.tsv is missing; run \
           tools/build_ecma262_regexp_match_engine_exact_plan.py"
      else climb parent
  in
  climb cwd
  ...
  List.iter
    (fun row ->
      ...
      if field "observability_reason" row = "" then
        Alcotest.failf "%s: observability_reason is empty" (field "plan_id" row);
      ...
      if field "plan_state" row = "planned_not_executable" then begin
        ...
        then
          Alcotest.failf "%s: exact_case_id has wrong executable prefix"
            (field "plan_id" row);
        ...
        Alcotest.(check string)
          "observable executable row" "search_bool_observable"
          (field "observability_status" row);
        Alcotest.(check string)
          "executable next action" "materialize_match_engine_exact_case"
        ...
        if
          not
            (String.starts_with ~prefix:"match-engine-deferred:"
               (field "exact_case_id" row))
        then
          Alcotest.failf "%s: exact_case_id has wrong deferred prefix"
            (field "plan_id" row);
        ...
        if
          ...
          && field "observability_status" row <> "requires_exec_result_observer"
        then
          Alcotest.failf "%s: invalid deferred observability_status %s"
            (field "plan_id" row)
            (field "observability_status" row)
      end)
    rows
