  ...
  List.iter
    (fun row ->
       ...
       if field "mapping_family" row = "unknown_mapping_family" then
         Alcotest.failf "%s: mapping_family is unknown" (field "requirement_id" row))
    rows

let () =
  Alcotest.run "ecma262-requirement-test-worklist" [
    ("manifest", [
      Alcotest.test_case "post-credit requirement-test worklist invariants" `Quick
        test_post_credit_worklist_manifest;
    ]);
  ]
