let repo_root () =
  ...
  let rec climb dir =
    let candidate = Filename.concat dir "cache/ecma262-regexp-test-evidence.tsv" in
...
let evidence_rows () =
  read_tsv [ "cache"; "ecma262-regexp-test-evidence.tsv" ]
...
let test_evidence_manifest () =
  ...
  List.iter
    (fun row ->
       ...
       then Alcotest.failf "%s: coverage credit must be none"
           (field "evidence_id" row);
       if field "case_id" row <> "" && not (source_exists (field "case_source" row))
...
let check_compile_evidence index total row =
  ...
  | "compile_error", Ok _ ->
    Alcotest.failf
      "evidence compile_error unexpectedly compiled (%d/%d): evidence=%s case=%s \
       source=%s pattern=%S flags=%S"
      (index + 1) total
      (field "evidence_id" row)
      (field "case_id" row)
      (field "case_source" row)
