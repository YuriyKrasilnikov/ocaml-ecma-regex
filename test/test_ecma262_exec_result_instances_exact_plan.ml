...
let repo_root () =
  ...
  let rec climb dir =
    ...
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-exec-result-instances-exact-plan.tsv is missing; \
           run tools/build_ecma262_regexp_exec_result_instances_exact_plan.py"
      else climb parent
  in
  climb cwd
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "exec-result instances exact plan rows" 3
    (List.length rows);
  Alcotest.(check int) "planned executable rows" 3
  ...
  check_count observability_counts
    "internal_exec_result_instance_model_observable"
    3;
  ...
  List.iter
    (fun row ->
       ...
       Alcotest.(check string)
         "next action"
         "materialize_exec_result_instances_exact_case"
...
let array_contains value items =
  Array.exists (( = ) value) items
...
let check_model_field row
  ...
  | "regexp_instance_internal_slots_observed" ->
    ...
    Alcotest.(check bool)
      "OriginalFlags slot"
      true
      observation.Core.original_flags_slot_observed;
    Alcotest.(check bool)
      "RegExpRecord slot"
      true
      observation.Core.regexp_record_slot_observed;
    Alcotest.(check bool)
      "RegExpMatcher slot"
      true
      observation.Core.regexp_matcher_slot_observed;
    Alcotest.(check bool)
      "RegExpMatcher closure"
      true
  ...
  | "regexp_instance_last_index_property_observed" ->
    Alcotest.(check bool)
      "lastIndex property"
      true
      observation.Core.last_index_property_observed
...
let test_exact_plan_exec_cases () =
  plan_rows ()
  |> planned_rows
  |> List.iter check_instance_case

let () =
  Alcotest.run "ecma262-exec-result-instances-exact-plan" [
    ("manifest", [
      Alcotest.test_case "exec-result instances exact plan invariants" `Quick
