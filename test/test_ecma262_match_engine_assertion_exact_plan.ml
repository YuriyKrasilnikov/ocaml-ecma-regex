let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir
        "cache/ecma262-regexp-match-engine-assertion-exact-plan.tsv"
    in
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-match-engine-assertion-exact-plan.tsv is \
           missing; run \
...
let plan_rows () =
  read_tsv [ "cache"; "ecma262-regexp-match-engine-assertion-exact-plan.tsv" ]
  ...
  List.filter
    (fun row ->
      ...
      && field "coverage_credit" row
         = "none_match_engine_assertion_exact_planned")
    rows
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "assertion exact plan rows" 89 (List.length rows);
  Alcotest.(check int)
    "planned executable rows" 89
    (List.length (planned_rows rows));
  let state_counts = count_by "plan_state" rows in
  check_count state_counts "planned_not_executable" 89;
  let credit_counts = count_by "coverage_credit" rows in
  check_count credit_counts "none_match_engine_assertion_exact_planned" 89;
  let mapping_counts = count_by "mapping_family" rows in
  check_count mapping_counts "match_engine_assertions" 89;
  let layer_counts = count_by "executable_layer" rows in
  check_count layer_counts "match_engine" 89;
  let search_counts = count_by "expected_search_result" rows in
  check_count search_counts "true" 83;
  check_count search_counts "false" 6;
  let exec_counts = count_by "expected_exec_result" rows in
  check_count exec_counts "true" 83;
  check_count exec_counts "false" 6;
  let family_counts = count_by "exact_case_family" rows in
  check_count family_counts "compile_assertion_dispatch" 2;
  check_count family_counts "start_anchor_generic" 6;
  check_count family_counts "end_anchor_generic" 7;
  check_count family_counts "word_boundary_success" 9;
  check_count family_counts "word_boundary_failure" 1;
  check_count family_counts "non_word_boundary_success" 9;
  check_count family_counts "non_word_boundary_failure" 1;
  check_count family_counts "positive_lookahead_success" 10;
  check_count family_counts "positive_lookahead_failure" 1;
  check_count family_counts "positive_lookahead_capture_visible" 5;
  check_count family_counts "negative_lookahead_success" 10;
  check_count family_counts "negative_lookahead_failure" 1;
  check_count family_counts "positive_lookbehind_success" 10;
  check_count family_counts "positive_lookbehind_failure" 1;
  check_count family_counts "positive_lookbehind_capture_visible" 5;
  check_count family_counts "negative_lookbehind_success" 10;
  check_count family_counts "negative_lookbehind_failure" 1;
  List.iter
    (fun row ->
      if
        not
...
let check_assertion_case row =
  let flags = flags_for row in
  match Ecma_regex.compile ~flags (field "pattern" row) with
  | Error msg ->
      Alcotest.failf
        "assertion exact case failed to compile: plan=%s requirement=%s \
...
let test_exact_plan_assertion_cases () =
  plan_rows () |> planned_rows |> List.iter check_assertion_case

let () =
  Alcotest.run "ecma262-match-engine-assertion-exact-plan"
    [
      ( "manifest",
        [
          Alcotest.test_case "assertion exact plan invariants" `Quick
            test_exact_plan_manifest;
        ] );
      ( "match",
        [
          Alcotest.test_case "assertion exact planned cases" `Quick
            test_exact_plan_assertion_cases;
        ] );
    ]
