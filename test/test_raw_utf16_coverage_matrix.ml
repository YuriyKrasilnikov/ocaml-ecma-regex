...
let test_summary_matches_matrix () =
  ...
  check_summary_int "covered_artifact_test_raw_utf16_negative_position_matrix_ml" 10;
  ...
  Alcotest.(check string)
    "open rows require reason"
    "true"
    (summary_value "open_rows_require_reason");
  Alcotest.(check string)
    "covered rows require executable gate"
    "true"
    (summary_value "covered_rows_require_executable_gate");
  Alcotest.(check string)
    "non-applicable rows require reason"
    "true"
  ...
  List.iter
    (fun row ->
       ...
       | "covered" ->
         ...
         Alcotest.(check string) (row_id ^ ": next_action") "none" (field "next_action" row)
       | "open" ->
         ...
         if field "next_action" row = "none" then
           Alcotest.failf "%s: open row must have a concrete next_action" row_id
...
let () =
  Alcotest.run
    ...
    , [ Alcotest.test_case "schema and counts" `Quick test_schema_and_counts
      ...
      ; Alcotest.test_case "covered artifact counts" `Quick test_covered_artifact_counts
      ]
    ]
;;
