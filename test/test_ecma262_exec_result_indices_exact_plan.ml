...
let repo_root () =
  ...
  let rec climb dir =
    ...
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-exec-result-indices-exact-plan.tsv is missing; \
           run tools/build_ecma262_regexp_exec_result_indices_exact_plan.py"
      else climb parent
  in
  climb cwd
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int)
    "exec-result indices exact plan rows" 35 (List.length rows);
  Alcotest.(check int)
    "planned executable rows" 35
  ...
  check_count observability_counts
    "internal_exec_result_indices_model_observable" 35;
  let target_counts = count_by "target_test_artifact" rows in
  check_count target_counts
    "test/test_ecma262_exec_result_indices_exact_plan.ml" 35;
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "next action" "materialize_exec_result_indices_exact_case"
...
let check_model_field row observation =
  ...
  | "group_name_read" ->
      check_bool_model_field "group name read" true
        observation.Core.group_name_read
...
let check_index_entry row observation =
  ...
  if
    entry_index < 0
    || entry_index >= Array.length observation.Core.exec_result_indices
  then
    Alcotest.failf "%s: index entry %d is out of range" (field "plan_id" row)
      entry_index;
  let pair = observation.Core.exec_result_indices.(entry_index) in
  if bool_field row "expected_index_pair_defined" then
    begin match pair with
    | None ->
        Alcotest.failf "%s: expected defined index pair at %d"
          (field "plan_id" row) entry_index
    | Some pair ->
        Alcotest.(check string)
          "index pair start"
  ...
  if entry_index > 0 then begin
    let group_name =
      observation.Core.exec_result_group_names.(entry_index - 1)
    in
    Alcotest.(check string)
      "group name"
...
let check_indices_case row =
  ...
  | Ok regexp ->
      ...
      in
      Alcotest.(check bool)
        "has indices flag" true observation.Core.has_indices_flag;
      Alcotest.(check int)
        "indices length"
        (parse_int_field row "expected_indices_length")
        observation.Core.indices_array_length;
...
let test_exact_plan_indices_cases () =
  plan_rows () |> planned_rows |> List.iter check_indices_case
