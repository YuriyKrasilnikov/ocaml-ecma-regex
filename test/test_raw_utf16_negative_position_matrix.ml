...
let instance_case name ~flags ~last_index ~expected_last_index pattern input_units
    expected =
  { instance_name = name
...
let run_exec_case
  ...
  check_expected_result exec_name exec_expected (Ecma_regex.exec_js regexp input)
;;

let run_search_case
    { search_name; search_pattern; search_flags; search_input_units; search_expected }
    () =
...
let exec_cases =
  ...
  ; exec_case
      ~flags:"u"
      "exec /u matches lone low surrogate after BMP prefix"
      "\\uDE00"
      a_then_low
  ...
  ; exec_case
      "exec without Unicode can search low surrogate inside pair"
      "\\uDE00"
      grinning_face
      (match_ ~start_index:1 [ low ])
  ; exec_case
  ...
  ; exec_case
      ~flags:"u"
      "exec /u boundary plus property complement does not match inside emoji pair"
      "\\B\\P{Emoji}"
      grinning_then_a
      No_match
  ; exec_case
      ~flags:"u"
      "exec /u word boundary skips pair and appears before A"
      "\\b"
      grinning_then_a
...
let search_cases =
  ...
  ; search_case
      ~flags:"u"
      "search_js /u boundary-property complement avoids inside-pair false positive"
      "\\B\\P{Emoji}"
      grinning_then_a
      false
  ]
...
let instance_cases =
  [ instance_case
      "global /gu low surrogate search skips valid pair and resets"
      ~flags:"gu"
      ~last_index:0
      ...
      No_match
  ; instance_case
      "global /g low surrogate search can match inside pair"
      ~flags:"g"
      ~last_index:0
      ...
      (match_ ~start_index:1 [ low ])
  ; instance_case
      "sticky /uy low surrogate at pair start rejects and resets"
      ~flags:"uy"
      ~last_index:0
      ~expected_last_index:0
      "\\uDE00"
      ...
      No_match
  ; instance_case
      "sticky /uy low surrogate at explicit low index matches"
      ~flags:"uy"
      ~last_index:1
      ~expected_last_index:2
      "\\uDE00"
      ...
      (match_ ~start_index:1 [ low ])
  ; instance_case
      "sticky /uy dot at pair start consumes full pair"
      ~flags:"uy"
      ~last_index:0
      ~expected_last_index:2
      "."
  ...
  ; instance_case
      "sticky /uy dot at explicit low index consumes low surrogate"
      ~flags:"uy"
      ~last_index:1
      ~expected_last_index:2
      "."
  ...
  ; instance_case
      ...
      (match_ ~start_index:1 [ low ])
  ; instance_case
      "global /gu property complement skips emoji pair before A"
      ~flags:"gu"
      ~last_index:0
...
let test_unicode_empty_iterator_advances_over_pair () =
  ...
  check_expected_result
    "iterator /gu empty at pair start"
    (match_ ~end_index:0 [])
    (Ecma_regex.next_match_js iterator);
  Alcotest.(check int)
    "iterator /gu last_index after pair-start empty"
    2
    (Ecma_regex.last_index instance)
;;
...
let test_non_unicode_empty_iterator_advances_one_code_unit () =
  ...
  let iterator = Ecma_regex.iter_matches_js instance input in
  check_expected_result
    "iterator /g empty at pair start"
    (match_ ~end_index:0 [])
    (Ecma_regex.next_match_js iterator);
  Alcotest.(check int)
    "iterator /g last_index after pair-start empty"
    1
    (Ecma_regex.last_index instance)
;;
...
let test_unicode_empty_iterator_from_low_index_advances_one_code_unit () =
  ...
  let iterator = Ecma_regex.iter_matches_js instance input in
  check_expected_result
    "iterator /gu empty at explicit low index"
    (match_ ~start_index:1 ~end_index:1 [])
    (Ecma_regex.next_match_js iterator);
  Alcotest.(check int)
    "iterator /gu last_index after explicit low-index empty"
    2
    (Ecma_regex.last_index instance)
;;
...
let test_unicode_iterator_does_not_find_low_inside_pair () =
  ...
  let iterator = Ecma_regex.iter_matches_js instance input in
  check_expected_result
    "iterator /gu low surrogate inside pair"
    No_match
    (Ecma_regex.next_match_js iterator);
  Alcotest.(check int)
    "iterator /gu low surrogate miss resets last_index"
    0
    (Ecma_regex.last_index instance)
;;
...
let exec_tests =
  List.map
    (fun test_case -> Alcotest.test_case test_case.exec_name `Quick (run_exec_case test_case))
    exec_cases
;;

let search_tests =
  List.map
    (fun test_case -> Alcotest.test_case test_case.search_name `Quick (run_search_case test_case))
    search_cases
;;
...
let instance_tests =
  List.map
    (fun test_case ->
       Alcotest.test_case test_case.instance_name `Quick (run_instance_case test_case))
    instance_cases
;;

let iterator_tests =
  [ Alcotest.test_case
      "iterator /gu empty advances over valid surrogate pair"
      `Quick
      test_unicode_empty_iterator_advances_over_pair
  ; Alcotest.test_case
      "iterator /g empty advances one raw code unit"
      `Quick
      test_non_unicode_empty_iterator_advances_one_code_unit
  ; Alcotest.test_case
      "iterator /gu empty from explicit low index advances one code unit"
      `Quick
      test_unicode_empty_iterator_from_low_index_advances_one_code_unit
  ; Alcotest.test_case
      "iterator /gu does not find low surrogate inside pair"
      `Quick
      test_unicode_iterator_does_not_find_low_inside_pair
  ]
;;

let () =
  Alcotest.run
    "raw-utf16-negative-position-matrix"
