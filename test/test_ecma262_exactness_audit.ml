let repo_root () =
  ...
  let rec climb dir =
    let candidate = Filename.concat dir "cache/ecma262-regexp-exactness-audit.tsv" in
...
let audit_rows () =
  read_tsv [ "cache"; "ecma262-regexp-exactness-audit.tsv" ]
...
let test_exactness_manifest () =
  ...
  check_count kind_counts "match_engine_unicode_sets_escape_string_exact_case" 16;
  ...
  List.iter
    (fun row ->
       ...
       then Alcotest.failf "%s: unexpected non-local-exact coverage credit"
           (field "audit_id" row);
       if
         field "case_source" row <> ""
         && not (source_exists (field "case_source" row))
       then Alcotest.failf "%s: missing test262 source %s"
           (field "audit_id" row)
           (field "case_source" row);
       if field "exactness_audit_state" row = "potential_exact_ready_manual_review"
        then Alcotest.failf "%s: exact-ready rows are intentionally zero in v1"
            (field "audit_id" row);
       if field "evidence_kind" row = "compile_parser_exact_case" then begin
         Alcotest.(check string)
           "compile/parser exact state"
         ...
         Alcotest.(check string)
           "compile/parser exact reuse count"
           "1"
         ...
         if
           not
             (String.starts_with ~prefix:"compile-parser-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: compile/parser exact case_id must use compile-parser-exact prefix"
             (field "audit_id" row);
         if
           not
       ...
       if field "evidence_kind" row = "literal_lexer_exact_case" then begin
         Alcotest.(check string)
           "literal lexer exact state"
           "covered_by_literal_lexer_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
           "literal lexer exact credit"
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "literal lexer exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"literal-lexer-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: literal lexer exact case_id must use literal-lexer-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "match_engine_atoms_exact_case" then begin
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine atom exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-atoms-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine atom exact case_id must use match-engine-atoms-exact prefix"
             (field "audit_id" row);
       if
         not
           (String.starts_with ~prefix:"external/ecma262/"
      ...
      if field "evidence_kind" row = "match_engine_capture_exact_case" then begin
        Alcotest.(check string)
          "match-engine capture exact state"
          "covered_by_match_engine_exact"
          (field "exactness_audit_state" row);
        Alcotest.(check string)
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine capture exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
        ...
        if
          not
            (String.starts_with ~prefix:"match-engine-capture-exact:"
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine capture exact case_id must use match-engine-capture-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
      ...
      then begin
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine UnicodeSets string exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
        ...
        if
          not
            (String.starts_with
               ...
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine UnicodeSets string exact case_id must use match-engine-unicode-sets-string-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
               (field "case_source" row))
        then Alcotest.failf
            "%s: match-engine UnicodeSets string exact case_source must use ECMA source"
            (field "audit_id" row)
      end;
      if
        field "evidence_kind" row
      ...
      then begin
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine UnicodeSets escape string exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
        ...
        if
          not
            (String.starts_with
               ...
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine UnicodeSets escape string exact case_id must use match-engine-unicode-sets-escape-string-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
               (field "case_source" row))
        then Alcotest.failf
            "%s: match-engine UnicodeSets escape string exact case_source must use ECMA source"
            (field "audit_id" row)
      end;
      if
        field "evidence_kind" row
      ...
      then begin
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine character-class exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
          ...
          (field "selected_missing_selector_tags" row);
        Alcotest.(check string)
          "match-engine character-class exact expected behavior present"
          "true"
          (string_of_bool
             (field "expected_behavior" row
        ...
        if
          not
            (String.starts_with
               ...
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine character-class exact case_id must use match-engine-character-classes-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
               (field "case_source" row))
        then Alcotest.failf
            "%s: match-engine character-class exact case_source must use ECMA source"
            (field "audit_id" row)
      end;
      if field "evidence_kind" row = "match_engine_concatenation_exact_case" then begin
        Alcotest.(check string)
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine MatchSequence exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
        ...
        if
          not
            (String.starts_with ~prefix:"match-engine-concatenation-exact:"
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine MatchSequence exact case_id must use match-engine-concatenation-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
               (field "case_source" row))
        then Alcotest.failf
            "%s: match-engine MatchSequence exact case_source must use ECMA source"
            (field "audit_id" row)
      end;
      if field "evidence_kind" row = "match_engine_backreference_exact_case" then begin
        Alcotest.(check string)
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine backreference exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
        ...
        if
          not
            (String.starts_with ~prefix:"match-engine-backreference-exact:"
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine backreference exact case_id must use match-engine-backreference-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
               (field "case_source" row))
        then Alcotest.failf
            "%s: match-engine backreference exact case_source must use ECMA source"
            (field "audit_id" row)
      end;
      if
        field "evidence_kind" row
      ...
      then begin
        ...
        Alcotest.(check string)
          ...
          (field "next_action" row);
        Alcotest.(check string)
          "match-engine BackreferenceMatcher exact reuse count"
          "1"
          (field "case_reuse_count" row);
        Alcotest.(check string)
        ...
        if
          not
            (String.starts_with
               ...
               (field "case_id" row))
        then Alcotest.failf
            "%s: match-engine BackreferenceMatcher exact case_id must use match-engine-backreference-matcher-exact prefix"
            (field "audit_id" row);
        if
          not
            (String.starts_with ~prefix:"external/ecma262/"
               (field "case_source" row))
        then Alcotest.failf
            "%s: match-engine BackreferenceMatcher exact case_source must use ECMA source"
            (field "audit_id" row)
      end;
      if field "evidence_kind" row = "match_engine_exact_case" then begin
         Alcotest.(check string)
           "match-engine exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
           "match-engine exact credit"
           "match_engine_exact_requirement_credit"
           (field "coverage_credit" row);
         Alcotest.(check string)
           "match-engine exact next action"
           "none_covered_by_match_engine_exact"
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         then Alcotest.failf
             "%s: match-engine exact expected behavior must be search_true or search_false"
             (field "audit_id" row);
         if
           not
       ...
       if field "evidence_kind" row = "match_engine_result_exact_case" then begin
         Alcotest.(check string)
           "match-engine result exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine result exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-result-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine result exact case_id must use match-engine-result-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "exec_result_matching_exact_case" then begin
         Alcotest.(check string)
           "exec-result matching exact state"
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "exec-result matching exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "exec-result matching exact expected behavior present"
           "true"
           (string_of_bool
         ...
         if
           not
             (String.starts_with ~prefix:"exec-result-matching-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: exec-result matching exact case_id must use exec-result-matching-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "exec_result_exec_exact_case" then begin
         Alcotest.(check string)
           "exec-result exec exact state"
           "covered_by_exec_result_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
           "exec-result exec exact credit"
           "exec_result_exact_requirement_credit"
           (field "coverage_credit" row);
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "exec-result exec exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "exec-result exec exact expected behavior present"
           "true"
           (string_of_bool
              (List.mem (field "expected_behavior" row)
                 [
         ...
         if
           not
             (String.starts_with ~prefix:"exec-result-exec-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: exec-result exec exact case_id must use exec-result-exec-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "exec_result_capture_exact_case" then begin
         Alcotest.(check string)
           "exec-result capture exact state"
           "covered_by_exec_result_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "exec-result capture exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "exec-result capture exact expected behavior present"
           "true"
           (string_of_bool
              (List.mem (field "expected_behavior" row)
                 [
         ...
         if
           not
             (String.starts_with ~prefix:"exec-result-capture-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: exec-result capture exact case_id must use exec-result-capture-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "exec_result_indices_exact_case" then begin
         Alcotest.(check string)
           "exec-result indices exact state"
           "covered_by_exec_result_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "exec-result indices exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "exec-result indices exact expected behavior present"
           "true"
           (string_of_bool
              (List.mem (field "expected_behavior" row)
                 [
         ...
         if
           not
             (String.starts_with ~prefix:"exec-result-indices-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: exec-result indices exact case_id must use exec-result-indices-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "exec_result_instances_exact_case" then begin
         Alcotest.(check string)
           "exec-result instances exact state"
           "covered_by_exec_result_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "exec-result instances exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "exec-result instances exact expected behavior present"
           "true"
           (string_of_bool
              (List.mem (field "expected_behavior" row)
                 [
         ...
         if
           not
             (String.starts_with ~prefix:"exec-result-instances-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: exec-result instances exact case_id must use exec-result-instances-exact prefix"
             (field "audit_id" row);
       if
         not
           (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "spec_model_exact_case" then begin
         Alcotest.(check string)
           "spec-model exact state"
           "covered_by_spec_model_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
           "spec-model exact credit"
           "spec_model_exact_requirement_credit"
           (field "coverage_credit" row);
         Alcotest.(check string)
           "spec-model exact next action"
           "none_covered_by_spec_model_exact"
           (field "next_action" row);
         Alcotest.(check string)
           "spec-model exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "spec-model exact expected behavior present"
           "true"
           (string_of_bool
              (List.mem (field "expected_behavior" row)
                 [
         ...
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
                (field "case_source" row))
         then Alcotest.failf
             "%s: spec-model exact case_source must use ECMA source"
             (field "audit_id" row)
       end;
       if field "evidence_kind" row = "test262_literal_lexer_exact_case" then begin
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "test262 literal-lexer exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"test262-regexp-executable:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: test262 literal-lexer exact case_id must use test262-regexp-executable prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"test/"
                (field "case_source" row))
         then Alcotest.failf
             "%s: test262 literal-lexer exact case_source must use test262 source"
             (field "audit_id" row)
       end;
       if field "evidence_kind" row = "match_engine_start_anchor_exact_case" then begin
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine start-anchor exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-start-anchor-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine start-anchor exact case_id must use match-engine-start-anchor-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
                (field "case_source" row))
         then Alcotest.failf
             "%s: match-engine start-anchor exact case_source must use ECMA source"
             (field "audit_id" row)
       end;
       if field "evidence_kind" row = "match_engine_end_anchor_exact_case" then begin
         Alcotest.(check string)
           "match-engine end-anchor exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine end-anchor exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-end-anchor-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine end-anchor exact case_id must use match-engine-end-anchor-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "match_engine_quantifier_exact_case" then begin
         Alcotest.(check string)
           "match-engine quantifier exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine quantifier exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-quantifier-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine quantifier exact case_id must use match-engine-quantifier-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "match_engine_modifier_exact_case" then begin
         Alcotest.(check string)
           "match-engine modifier exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine modifier exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-modifier-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine modifier exact case_id must use match-engine-modifier-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "match_engine_assertion_exact_case" then begin
         Alcotest.(check string)
           "match-engine assertion exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine assertion exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-assertion-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine assertion exact case_id must use match-engine-assertion-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       then begin
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine Pattern Semantics exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with
                ...
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine Pattern Semantics exact case_id must use match-engine-pattern-semantics-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
                (field "case_source" row))
       then Alcotest.failf
             "%s: match-engine Pattern Semantics exact case_source must use ECMA source"
             (field "audit_id" row)
       end;
       if field "evidence_kind" row = "match_engine_annex_b_exact_case" then begin
         Alcotest.(check string)
           "match-engine Annex B exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
         ...
         Alcotest.(check string)
           ...
           (field "next_action" row);
         Alcotest.(check string)
           "match-engine Annex B exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         if
           not
             (String.starts_with ~prefix:"match-engine-annex-b-exact:"
                (field "case_id" row))
         then Alcotest.failf
             "%s: match-engine Annex B exact case_id must use match-engine-annex-b-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
       ...
       if field "evidence_kind" row = "match_state_exact_case" then begin
         Alcotest.(check string)
           "match-state exact state"
           "covered_by_match_engine_exact"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
           "match-state exact credit"
           "match_engine_exact_requirement_credit"
           (field "coverage_credit" row);
         Alcotest.(check string)
           "match-state exact next action"
           "none_covered_by_match_engine_exact"
           (field "next_action" row);
         Alcotest.(check string)
           "match-state exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
           ...
           (field "selected_missing_selector_tags" row);
         Alcotest.(check string)
           "match-state exact expected behavior"
           "match_state_model_observable"
           (field "expected_behavior" row);
         if
           not
       ...
       if field "evidence_kind" row = "local_exact_compile_parser_case" then begin
         ...
         Alcotest.(check string)
           ...
           (field "coverage_credit" row);
         Alcotest.(check string)
           "local exact next action"
           "none_covered_by_local_exact_compile_parser"
           (field "next_action" row);
         Alcotest.(check string)
           "local exact reuse count"
           "1"
           (field "case_reuse_count" row);
         Alcotest.(check string)
         ...
         then Alcotest.failf "%s: local exact case_id must use local-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
                (field "case_source" row))
         then Alcotest.failf "%s: local exact case_source must use ECMA source"
             (field "audit_id" row)
       end;
       if field "evidence_kind" row = "reused_candidate_exact_compile_parser_case" then begin
         Alcotest.(check string)
           "reused exact state"
         ...
         Alcotest.(check string)
           ...
           ""
           (field "selected_missing_selector_tags" row);
         if not (String.starts_with ~prefix:"reused-exact:" (field "case_id" row))
         then Alcotest.failf "%s: reused exact case_id must use reused-exact prefix"
             (field "audit_id" row);
         if
           not
             (String.starts_with ~prefix:"external/ecma262/"
                (field "case_source" row))
         then Alcotest.failf "%s: reused exact case_source must use ECMA source"
             (field "audit_id" row)
       end;
       if field "evidence_kind" row = "open_reused_candidate_manual_spec_review" then begin
         Alcotest.(check string)
           "manual reused state"
           "open_reused_candidate_manual_spec_review_required"
           (field "exactness_audit_state" row);
         Alcotest.(check string)
           "manual reused credit"
