...
let repo_root () =
  ...
  let rec climb dir =
    ...
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-exec-result-capture-exact-plan.tsv is missing; \
           run tools/build_ecma262_regexp_exec_result_capture_exact_plan.py"
      else climb parent
  in
  climb cwd
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int)
    "exec-result capture exact plan rows" 13 (List.length rows);
  Alcotest.(check int)
  ...
  check_count observability_counts
    "internal_exec_result_capture_model_observable" 13;
  let target_counts = count_by "target_test_artifact" rows in
  check_count target_counts
    "test/test_ecma262_exec_result_capture_exact_plan.ml" 13;
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "next action" "materialize_exec_result_capture_exact_case"
...
let option_string_to_string = function None -> "" | Some value -> value
...
let check_capture row observation =
  ...
  if index < 0 || index >= Array.length observation.Core.exec_result_captures
  then
    Alcotest.failf "%s: capture ordinal %d is out of range"
      (field "plan_id" row) ordinal;
...
let test_exact_plan_capture_cases () =
  plan_rows () |> planned_rows |> List.iter check_capture_case
