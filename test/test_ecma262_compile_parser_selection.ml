...
let negative_or_local_exact_rows rows =
  List.filter
    (fun row -> field "selection_state" row = "needs_negative_or_local_exact_case")
    rows
...
let test_selection_manifest () =
  ...
  Alcotest.(check int) "unique selected compile cases" 0 (List.length unique_cases);
  List.iter
    (fun row ->
       if field "selected_case_id" row = "" then
         Alcotest.failf "%s: selected_case_id is empty" (field "requirement_id" row);
       if field "selected_pattern" row = "" then
         Alcotest.failf "%s: selected_pattern is empty" (field "requirement_id" row);
       ...
       Alcotest.(check string)
         "selection exactness"
         "selected_candidate_not_coverage"
  ...
  List.iter
    (fun row ->
       Alcotest.(check string)
         "negative/local expected behavior"
         "compile_error_or_local_exact_needed"
         (field "selected_expected_behavior" row);
       Alcotest.(check string)
         "negative/local exactness"
         "no_positive_exact_selection"
         (field "selection_exactness" row);
       Alcotest.(check string) "no selected case" "" (field "selected_case_id" row);
       if not (has_csv (field "candidate_selector_tags" row) "negative_syntax_needed")
...
let compile_selected_case index total row =
  ...
  | Error msg ->
    Alcotest.failf
      ...
      (field "selected_case_id" row)
      (field "selected_case_source" row)
      (field "selected_raw" row)
      pattern selected_flags msg

let test_selected_compile_positive_cases () =
  let selected =
    selection_rows ()
    |> selected_compile_positive_rows
