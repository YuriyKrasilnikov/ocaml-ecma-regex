...
let split_clause row =
  field "clause_id" row = "22.1.3.23" || field "clause_id" row = "22.2.6.14"
...
let test_product_surface_split_rows () =
  check_ids "split adapter rows"
    ...
    split_adapter_rows;
  check_ids "split non-applicable rows"
    [
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "split adapter test artifact" "test/test_ecma262_split_adapter.ml"
        (field "next_test_artifact" row);
      Alcotest.(check string)
        "split adapter public API status" "current_public_api"
        (field "public_api_status" row))
    split_adapter_rows;
  List.iter
    (fun row ->
      Alcotest.(check string)
        "split JS protocol artifact" "none"
        (field "ocaml_artifact" row);
      Alcotest.(check string)
        "split JS protocol status" "not_ocaml_surface"
        (field "public_api_status" row))
    split_nonapp_rows
...
let test_split_instance_uses_fresh_sticky_splitter_without_mutating_receiver ()
  ...
  Alcotest.(check (list string))
    "instance split ignores receiver lastIndex"
    [ "text:a"; "text:b"; "text:c" ]
    (Ecma_regex.split_instance instance "a,b,c" |> render_split_parts);
  Alcotest.(check int)
    "receiver lastIndex preserved" 2
...
let () =
  Alcotest.run "ecma262-split-adapter"
    [
      ( "manifest",
        [
          Alcotest.test_case "product-surface split rows" `Quick
            test_product_surface_split_rows;
        ] );
      ( "adapter semantics",
        [
          Alcotest.test_case "split returns substrings and respects limit"
            `Quick test_split_returns_substrings_and_respects_limit;
          Alcotest.test_case "split empty input uses RegExpExec result" `Quick
            test_split_empty_input_uses_regexp_exec_result;
          Alcotest.test_case
            "split empty matches advance with Unicode semantics" `Quick
            test_split_empty_matches_advance_with_unicode_semantics;
          Alcotest.test_case "split inserts captures" `Quick
            test_split_inserts_numbered_captures_and_undefined_captures;
          Alcotest.test_case "split instance preserves receiver" `Quick
            test_split_instance_uses_fresh_sticky_splitter_without_mutating_receiver;
          Alcotest.test_case "split_js preserves raw UTF-16 slices" `Quick
