...
let search_clause row =
  field "clause_id" row = "22.1.3.21"
  || field "clause_id" row = "22.2.6.12"
...
let test_search_index_respects_sticky_from_zero () =
  ...
  Alcotest.(check int) "sticky does not search past zero" (-1)
    (Ecma_regex.search_index later "ab");
  let at_zero = compile_or_fail ~flags "a" in
  Alcotest.(check int) "sticky match at zero" 0
...
let test_search_index_js_returns_utf16_index () =
  ...
  Alcotest.(check int) "JS string UTF-16 start index" 2
    (Ecma_regex.search_index_js regexp input);
  let flags = flags_or_fail "u" in
  let low_surrogate = compile_or_fail ~flags "\\uDE00" in
  Alcotest.(check int) "Unicode search skips inside surrogate pair" (-1)
...
let test_search_instance_index_restores_last_index () =
  ...
  Alcotest.(check int) "global instance search index" 1
    (Ecma_regex.search_instance_index instance "abc");
  Alcotest.(check int) "global instance lastIndex restored" 2
  ...
  Alcotest.(check int) "sticky instance search starts at zero" (-1)
    (Ecma_regex.search_instance_index sticky "ab");
  Alcotest.(check int) "sticky instance lastIndex restored" 1
...
let test_search_instance_index_js_restores_last_index () =
  ...
  Alcotest.(check int) "JS instance search index" 2
    (Ecma_regex.search_instance_index_js instance input);
  Alcotest.(check int) "JS instance lastIndex restored" 3
    (Ecma_regex.last_index instance)

let () =
  Alcotest.run
    "ecma262-search-adapter"
    [ ( "manifest"
      , [ Alcotest.test_case
            "product-surface search rows"
            `Quick
            test_product_surface_search_rows
        ] )
    ; ( "adapter semantics"
      , [ Alcotest.test_case
            "search_index returns match start or -1"
            `Quick
            test_search_index_returns_match_start_or_minus_one
        ; Alcotest.test_case
            "search_index respects sticky from zero"
            `Quick
            test_search_index_respects_sticky_from_zero
        ; Alcotest.test_case
            "search_index_js returns UTF-16 index"
            `Quick
            test_search_index_js_returns_utf16_index
        ; Alcotest.test_case
            "search_instance_index restores lastIndex"
            `Quick
            test_search_instance_index_restores_last_index
        ; Alcotest.test_case
            "search_instance_index_js restores lastIndex"
            `Quick
            test_search_instance_index_js_restores_last_index
        ] )
    ]
