let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir
        "cache/ecma262-regexp-match-engine-end-anchor-exact-plan.tsv"
    in
    if Sys.file_exists candidate then dir
      ...
      if parent = dir then
        Alcotest.fail
          "cache/ecma262-regexp-match-engine-end-anchor-exact-plan.tsv is \
           missing; run \
...
let plan_rows () =
  read_tsv [ "cache"; "ecma262-regexp-match-engine-end-anchor-exact-plan.tsv" ]
  ...
  List.filter
    (fun row ->
      ...
      && field "coverage_credit" row
         = "none_match_engine_end_anchor_exact_planned")
    rows
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "end-anchor exact plan rows" 6 (List.length rows);
  Alcotest.(check int)
  ...
  check_count credit_counts "none_match_engine_end_anchor_exact_planned" 6;
  ...
  check_count behavior_counts "end_anchor_zero_width_match_at_input_end" 1;
  check_count behavior_counts "end_anchor_condition_input_end_success" 1;
  check_count behavior_counts "end_anchor_condition_failure_blocks_search_loop"
    1;
  check_count behavior_counts "end_anchor_multiline_line_terminator_success" 2;
  check_count behavior_counts "end_anchor_condition_failure_without_multiline" 1;
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "source failure family" "matcher_runtime_end_anchor"
...
let check_end_anchor_case row =
  let flags = flags_for row in
  match Ecma_regex.compile ~flags (field "pattern" row) with
  | Error msg ->
      Alcotest.failf
        "end-anchor exact case failed to compile: plan=%s requirement=%s \
...
let test_exact_plan_end_anchor_cases () =
  plan_rows () |> planned_rows |> List.iter check_end_anchor_case

let () =
  Alcotest.run "ecma262-match-engine-end-anchor-exact-plan"
    [
      ( "manifest",
        [
          Alcotest.test_case "end-anchor exact plan invariants" `Quick
            test_exact_plan_manifest;
        ] );
      ( "match",
        [
          Alcotest.test_case "end-anchor exact planned cases" `Quick
            test_exact_plan_end_anchor_cases;
        ] );
    ]
