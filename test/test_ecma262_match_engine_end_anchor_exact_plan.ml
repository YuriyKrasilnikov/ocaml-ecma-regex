...
let plan_rows () =
  read_tsv [
    "cache";
    "ecma262-regexp-match-engine-end-anchor-exact-plan.tsv";
  ]
...
let parse_nonnegative_int_field row name =
  ...
  if value < 0 then
    Alcotest.failf "%s: %s must be non-negative"
      (field "plan_id" row)
      name;
  value
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "planned executable rows" 6
  ...
  check_count behavior_counts
    "end_anchor_zero_width_match_at_input_end" 1;
  check_count behavior_counts
    "end_anchor_condition_input_end_success" 1;
  check_count behavior_counts
    "end_anchor_condition_failure_blocks_search_loop" 1;
  check_count behavior_counts
    "end_anchor_multiline_line_terminator_success" 2;
  check_count behavior_counts
    "end_anchor_condition_failure_without_multiline" 1;
  ...
  List.iter
    (fun row ->
       ...
       Alcotest.(check string)
         "source failure family"
         "matcher_runtime_end_anchor"
...
let test_exact_plan_end_anchor_cases () =
  plan_rows ()
  |> planned_rows
  |> List.iter check_end_anchor_case
