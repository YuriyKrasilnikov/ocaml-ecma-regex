...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int)
    "UnicodeSets escape string exact plan rows" 16 (List.length rows);
  let planned = planned_rows rows in
  Alcotest.(check int) "planned executable rows" 16 (List.length planned);
  let state_counts = count_by "plan_state" rows in
  check_count state_counts "planned_not_executable" 16;
  let credit_counts = count_by "coverage_credit" rows in
  check_count credit_counts
    "none_match_engine_unicode_sets_escape_string_exact_planned" 16;
  let behavior_counts = count_by "expected_behavior" rows in
  check_count behavior_counts "unicode_sets_string_element_model_observable" 16;
  ...
  check_count mapping_counts "match_engine_atoms" 16;
  let layer_counts = count_by "executable_layer" rows in
  check_count layer_counts "match_engine" 16;
  let subfamily_counts = count_by "unicode_sets_subfamily" rows in
  check_count subfamily_counts
    "character_class_escape_unicode_sets_string_elements" 16;
  let route_counts = count_by "unicode_sets_semantic_route" rows in
  check_count route_counts "unicode_sets_string_element_matcher_model" 16;
  let observability_counts = count_by "observability_status" rows in
  check_count observability_counts
    "unicode_sets_string_element_model_observable" 16;
  List.iter
    (fun row ->
      if
        not
...
let observation_value observation = function
  ...
  | "unicode_sets_multi_char_elements_descending_iteration" ->
      observation
        .Core.unicode_sets_multi_char_elements_descending_iteration_observed
  | "unicode_sets_last_code_point_charset" ->
...
let check_unicode_sets_case row =
  ...
  | Error msg ->
      Alcotest.failf
        "UnicodeSets escape string exact case failed to compile: plan=%s \
  ...
  | Ok regexp ->
      ...
      | None -> Alcotest.failf "%s: expected exec match" (field "plan_id" row)
      | Some result ->
          Alcotest.(check string)
            "exec matched text"
...
let test_exact_plan_unicode_sets_cases () =
  plan_rows () |> planned_rows |> List.iter check_unicode_sets_case

let () =
  Alcotest.run "ecma262-match-engine-unicode-sets-escape-string-exact-plan"
    [
      ( "manifest",
        [
          Alcotest.test_case "UnicodeSets escape string exact plan invariants"
            `Quick test_exact_plan_manifest;
        ] );
      ( "model",
        [
          Alcotest.test_case "UnicodeSets escape string exact planned cases"
            `Quick test_exact_plan_unicode_sets_cases;
        ] );
    ]
