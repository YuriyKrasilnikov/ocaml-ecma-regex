...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "UnicodeSets escape string exact plan rows" 16
    (List.length rows);
...
let () =
  Alcotest.run "ecma262-match-engine-unicode-sets-escape-string-exact-plan" [
    ("manifest", [
      Alcotest.test_case "UnicodeSets escape string exact plan invariants" `Quick
        test_exact_plan_manifest;
    ]);
    ("model", [
      Alcotest.test_case "UnicodeSets escape string exact planned cases" `Quick
