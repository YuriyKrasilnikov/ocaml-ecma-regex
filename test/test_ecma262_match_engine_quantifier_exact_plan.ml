...
let plan_rows () =
  read_tsv [
    "cache";
    "ecma262-regexp-match-engine-quantifier-exact-plan.tsv";
  ]
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "planned executable rows" 45
...
let test_exact_plan_quantifier_cases () =
  plan_rows ()
  |> planned_rows
  |> List.iter check_quantifier_case
