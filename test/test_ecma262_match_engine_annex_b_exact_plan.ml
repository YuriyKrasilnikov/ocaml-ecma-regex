...
let test_exact_plan_manifest () =
  ...
  check_count target_counts
    "test/test_ecma262_match_engine_annex_b_exact_plan.ml" 49;
  List.iter
    (fun row ->
      if
        not
      ...
      Alcotest.(check string)
        "observable executable row" "search_and_exec_observable"
        (field "observability_status" row);
      Alcotest.(check string)
        "executable next action" "materialize_match_engine_annex_b_exact_case"
...
let compile_or_fail row =
  ...
  | Ok regexp -> regexp
  | Error msg ->
      Alcotest.failf "%s: compile failed: %s" (field "plan_id" row) msg
...
let test_exact_plan_cases () =
  ...
  List.iter
    (fun row ->
      ...
      | Some result, false ->
          Alcotest.failf "%s: unexpected exec match %S" (field "plan_id" row)
            result.matched_text
      | Some result, true ->
          Alcotest.(check int)
...
let () =
  Alcotest.run "ecma262-match-engine-annex-b-exact-plan"
    [
      ( "manifest",
        [
          Alcotest.test_case "Annex B exact plan invariants" `Quick
            test_exact_plan_manifest;
        ] );
      ( "exec",
        [
          Alcotest.test_case "Annex B exact planned cases" `Quick
