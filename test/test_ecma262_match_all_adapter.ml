...
let match_all_clause row =
  field "clause_id" row = "22.1.3.14" || field "clause_id" row = "22.2.6.9"
...
let test_product_surface_match_all_rows () =
  check_ids "matchAll adapter rows"
    [
  ...
  check_ids "matchAll non-applicable rows"
    [
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "matchAll adapter public API status" "current_public_api"
        (field "public_api_status" row))
    match_all_adapter_rows;
  List.iter
    (fun row ->
      Alcotest.(check string)
        "matchAll JS protocol artifact" "none"
        (field "ocaml_artifact" row);
      Alcotest.(check string)
        "matchAll JS protocol status" "not_ocaml_surface"
        (field "public_api_status" row))
    match_all_nonapp_rows
...
let test_match_all_instance_non_global_sticky_uses_last_index () =
  ...
  Alcotest.(check (list int))
    "sticky clone match at lastIndex" [ 1 ]
    (Ecma_regex.match_all_instance instance "ab" |> starts_of_match_results);
  Alcotest.(check int)
    "sticky original lastIndex preserved" 1
    (Ecma_regex.last_index instance);
  Ecma_regex.set_last_index instance 0;
  Alcotest.(check (list int))
    "sticky clone mismatch" []
    (Ecma_regex.match_all_instance instance "ab" |> starts_of_match_results);
  Alcotest.(check int)
    "sticky mismatch preserves original lastIndex" 0
    (Ecma_regex.last_index instance)
...
let test_match_all_instance_js_clones_last_index () =
  ...
  Alcotest.(check (list int))
    "JS clone starts from lastIndex" [ 2; 3 ]
    (Ecma_regex.match_all_instance_js instance input
    |> js_starts_of_match_results);
  Alcotest.(check int)
    "JS original lastIndex preserved" 2
    (Ecma_regex.last_index instance)

let () =
  Alcotest.run "ecma262-match-all-adapter"
    [
      ( "manifest",
        [
          Alcotest.test_case "product-surface matchAll rows" `Quick
            test_product_surface_match_all_rows;
        ] );
      ( "adapter semantics",
        [
          Alcotest.test_case "match_all non-global yields first result or empty"
            `Quick test_match_all_non_global_yields_first_result_or_empty;
          Alcotest.test_case "match_all global collects all results" `Quick
            test_match_all_global_collects_all_results;
          Alcotest.test_case
            "match_all global empty advances with unicode semantics" `Quick
            test_match_all_global_empty_advances_with_unicode_semantics;
          Alcotest.test_case "match_all_instance clones lastIndex" `Quick
            test_match_all_instance_clones_last_index;
          Alcotest.test_case
            "match_all_instance non-global sticky uses lastIndex" `Quick
            test_match_all_instance_non_global_sticky_uses_last_index;
          Alcotest.test_case "match_all_js returns raw UTF-16 slices" `Quick
            test_match_all_js_returns_raw_utf16_slices;
          Alcotest.test_case "match_all_js global exposes raw capture slices"
            `Quick test_match_all_js_global_exposes_raw_capture_slices;
          Alcotest.test_case
            "match_all_js empty advances over raw surrogate pair" `Quick
            test_match_all_js_empty_advances_over_raw_surrogate_pair;
          Alcotest.test_case "match_all_instance_js clones lastIndex" `Quick
