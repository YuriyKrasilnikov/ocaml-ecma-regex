...
let test_exact_plan_manifest () =
  ...
  check_count credit_counts "none_match_engine_character_classes_exact_planned"
    11;
  check_count credit_counts "none_match_engine_character_classes_exact_deferred"
    0;
  ...
  List.iter
    (fun row ->
      ...
      if field "character_class_subfamily" row = "character_range" then begin
        ...
        Alcotest.(check string)
          "next action" "materialize_match_engine_character_class_exact_case"
          (field "next_action" row)
      end
      else begin
        Alcotest.(check string)
          "complement expected observed" "true"
          (field "expected_observed" row);
        Alcotest.(check string)
...
let check_character_class_case row =
  ...
  | Ok regexp ->
      ...
      if field "character_class_subfamily" row = "character_complement" then begin
        Alcotest.(check bool)
          "AllCharacters code-unit universe observed" true
          observation
            .Core.character_complement_allcharacters_code_unit_universe_observed;
        Alcotest.(check bool)
          "AllCharacters code-point universe not claimed" false
          observation
            .Core
             .character_complement_allcharacters_code_point_universe_observed;
        Alcotest.(check bool)
          "AllCharacters case-fold-stable universe not claimed" false
          observation
            .Core
             .character_complement_allcharacters_case_fold_stable_universe_observed;
        Alcotest.(check bool)
          "CharacterComplement difference membership observed" true
          observation.Core.character_complement_difference_membership_observed;
        Alcotest.(check bool)
          "public search excludes set member" false (Core.search regexp "a")
      end

let test_exact_plan_character_class_cases () =
  plan_rows () |> planned_rows |> List.iter check_character_class_case
