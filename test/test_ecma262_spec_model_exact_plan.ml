...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "planned executable rows" 4
  ...
  List.iter
    (fun row ->
       ...
       Alcotest.(check string)
         "next action"
         "materialize_spec_model_exact_case"
...
let check_model_field row (observation : Core.spec_model_observation) =
  ...
  if not (array_contains expected_model_field observation.Core.observed_model_fields)
  then Alcotest.failf "%s: model field %S was not observed"
      (field "plan_id" row)
      expected_model_field;
  match expected_model_field with
  | "lexical_grammar_source_character_goal_model_observed" ->
    Alcotest.(check bool)
      "lexical terminal model"
      true
...
let check_spec_model_case row =
  ...
  Alcotest.(check string)
    "source text"
    (field "source_text" row)
    observation.Core.source_text;
  Alcotest.(check bool)
    "source fixture is ASCII"
    true
  ...
  Alcotest.(check string)
    ...
    (String.concat "," (Array.to_list observation.Core.lexical_grammar_goal_symbols));
  Alcotest.(check string)
    "regexp goal symbol"
