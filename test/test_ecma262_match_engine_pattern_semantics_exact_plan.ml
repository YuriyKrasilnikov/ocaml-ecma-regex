...
let decode_text source =
  ...
  let rec loop index =
    if index = String.length source then Buffer.contents buffer
      ...
      | other -> Alcotest.failf "unsupported escaped test input: \\\\%c" other);
      loop (index + 2)
    end
    else begin
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int) "pattern semantics exact plan rows" 56 (List.length rows);
  Alcotest.(check int)
    "planned executable rows" 56
    (List.length (executable_rows rows));
  Alcotest.(check int)
    "search/exec rows" 36
    (List.length (search_exec_rows rows));
  Alcotest.(check int)
    "internal model rows" 20
  ...
  check_count credit_counts "none_match_engine_pattern_semantics_exact_planned"
    56;
  check_count credit_counts "none_match_engine_pattern_semantics_exact_deferred"
    0;
  ...
  check_count observability_counts "internal_pattern_semantics_model_observable"
    20;
  ...
  List.iter
    (fun row ->
      ...
      if field "plan_state" row = "planned_not_executable" then begin
        ...
        | "internal_pattern_semantics_model_observable" ->
            Alcotest.(check string)
              "internal expected search" "not_applicable"
              (field "expected_search_result" row);
            Alcotest.(check string)
              "internal expected exec" "not_applicable"
              (field "expected_exec_result" row);
            if field "expected_model_field" row = "" then
              Alcotest.failf "%s: expected_model_field is empty"
                (field "plan_id" row);
            if field "model_scenario" row = "" then
              Alcotest.failf "%s: model_scenario is empty" (field "plan_id" row);
            Alcotest.(check string)
              "internal start" ""
        ...
        Alcotest.(check string)
          "deferred expected search" "not_observable"
          (field "expected_search_result" row);
        Alcotest.(check string)
...
let check_pattern_semantics_case row =
  ...
  | Error msg ->
      Alcotest.failf
        "Pattern Semantics exact case failed to compile: plan=%s \
...
let check_internal_model_invariants row
    (observation : Core.pattern_semantics_model_observation) =
  if observation.Core.input_code_point_count > observation.Core.input_length
  then
  ...
  | "quantifier_bounds_assert" ->
      Alcotest.(check (option int))
        "quantifier min" (Some 1) observation.Core.quantifier_min;
      Alcotest.(check (option int))
        "quantifier max" (Some 2) observation.Core.quantifier_max;
      Alcotest.(check (option bool))
        "quantifier greedy" (Some true) observation.Core.quantifier_greedy
  | "quantified_capture_index" ->
      Alcotest.(check (option int))
        "paren index" (Some 1) observation.Core.quantified_paren_index;
      Alcotest.(check (option int))
        "paren count" (Some 1) observation.Core.quantified_paren_count
  | "quantified_repeat_closure" ->
      Alcotest.(check (option int))
        "repeat min" (Some 0) observation.Core.quantifier_min;
      Alcotest.(check (option int))
        "repeat paren count" (Some 1) observation.Core.quantified_paren_count
  | "empty_matcher_state" -> (
...
let check_pattern_semantics_model_case row =
  ...
  | Error msg ->
      Alcotest.failf
        "Pattern Semantics model case failed to compile: plan=%s \
         requirement=%s pattern=%S flags=%S error=%s"
        (field "plan_id" row)
        (field "requirement_id" row)
        (field "pattern" row) (field "flags" row) msg
...
let test_exact_plan_pattern_semantics_cases () =
  plan_rows () |> search_exec_rows |> List.iter check_pattern_semantics_case

let test_exact_plan_pattern_semantics_model_cases () =
  plan_rows () |> internal_model_rows
