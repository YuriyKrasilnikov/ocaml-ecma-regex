...
let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir
        "cache/ecma262-regexp-match-engine-atoms-exact-plan.tsv"
    in
...
let search_rows rows =
  ...
  |> List.filter (fun row ->
      field "observability_status" row = "search_bool_observable")
...
let test_exact_plan_manifest () =
  ...
  Alcotest.(check int)
    "planned executable rows" 19
    (List.length (executable_rows rows));
  Alcotest.(check int)
    "search-observable rows" 17
    (List.length (search_rows rows));
  Alcotest.(check int)
    "CompileAtom model-observable rows" 2
  ...
  check_count state_counts "deferred_requires_compile_atom_piecewise_inventory"
    0;
  ...
  check_count state_counts "deferred_requires_unicode_sets_string_element_model"
    33;
    ...
    (fun row ->
      ...
      if field "plan_state" row = "planned_not_executable" then begin
        ...
        | "search_bool_observable" ->
            if
              field "expected_search_result" row <> "true"
        ...
        | "compile_atom_operation_model_observable" ->
            Alcotest.(check string)
              "CompileAtom model expected search" "model_observable"
              (field "expected_search_result" row);
            Alcotest.(check string)
              "CompileAtom model route" "operation_model"
              (field "atom_semantic_route" row)
        | other ->
            Alcotest.failf "%s: unexpected observability_status %S"
              (field "plan_id" row) other);
        Alcotest.(check string)
          "executable next action" "materialize_match_engine_atoms_exact_case"
...
let check_match index total row =
  ...
  | Error msg ->
      Alcotest.failf
        ...
        (index + 1) total (field "plan_id" row)
...
let check_compile_atom_model row =
  ...
  | Ok regexp ->
      ...
      Alcotest.(check bool)
        ...
        true
        (compile_atom_observation_value observation
           (field "expected_behavior" row))

let test_exact_plan_compile_atom_model_cases () =
  plan_rows () |> compile_atom_model_rows |> List.iter check_compile_atom_model
