...
let match_clause row =
  field "clause_id" row = "22.1.3.13" || field "clause_id" row = "22.2.6.8"
...
let texts_of_match_results = function
  | None -> None
  | Some results ->
      Some (List.map (fun result -> result.Ecma_regex.matched_text) results)

let starts_of_match_results = function
  | None -> None
  | Some results ->
      Some (List.map (fun result -> result.Ecma_regex.start_index) results)
...
let js_starts_of_match_results = function
  | None -> None
  | Some results ->
      Some (List.map (fun result -> result.Ecma_regex.js_start_index) results)
...
let single_js_match_result_or_fail = function
  ...
  | Some results ->
      Alcotest.failf "expected one JS match result, got %d"
        (List.length results)

let test_product_surface_match_rows () =
  check_ids "match adapter rows"
    [
    ...
    match_adapter_rows;
  check_ids "match non-applicable rows"
    [
  ...
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "match adapter test artifact" "test/test_ecma262_match_adapter.ml"
        (field "next_test_artifact" row);
      Alcotest.(check string)
        "match adapter public API status" "current_public_api"
        (field "public_api_status" row))
    match_adapter_rows;
  List.iter
    (fun row ->
      Alcotest.(check string)
        "match JS protocol artifact" "none"
        (field "ocaml_artifact" row);
      Alcotest.(check string)
        "match JS protocol status" "not_ocaml_surface"
        (field "public_api_status" row))
    match_nonapp_rows
...
let test_match_instance_non_global_sticky_uses_last_index () =
  ...
  Alcotest.(check (option (list int)))
    "sticky match at lastIndex" (Some [ 1 ])
    (Ecma_regex.match_instance instance "ab" |> starts_of_match_results);
  Alcotest.(check int)
    "sticky lastIndex after match" 2
    (Ecma_regex.last_index instance);
  Ecma_regex.set_last_index instance 0;
  Alcotest.(check (option (list int)))
    "sticky mismatch" None
    (Ecma_regex.match_instance instance "ab" |> starts_of_match_results);
  Alcotest.(check int)
    "sticky lastIndex after mismatch" 0
    (Ecma_regex.last_index instance)
...
let test_match_js_returns_raw_utf16_slices () =
  ...
  let low = 0xDE00 in
  let input =
    js_string_of_utf16_units_or_fail [ high; low; Char.code 'a'; high; low ]
  in
  let regexp = compile_or_fail ~flags "." in
...
let test_match_js_non_global_exposes_raw_capture_slices () =
  ...
  let low = 0xDE00 in
  let input =
    js_string_of_utf16_units_or_fail [ high; low; Char.code 'a'; Char.code 'z' ]
  in
  let regexp = compile_or_fail ~flags "(\\u{1F600})(?<tail>a)?" in
  let result =
    Ecma_regex.match_js regexp input |> single_js_match_result_or_fail
  in
...
let () =
  Alcotest.run "ecma262-match-adapter"
    [
      ( "manifest",
        [
          Alcotest.test_case "product-surface match rows" `Quick
            test_product_surface_match_rows;
        ] );
      ( "adapter semantics",
        [
          Alcotest.test_case "match_ non-global returns first exec result"
            `Quick test_match_non_global_returns_first_exec_result;
          Alcotest.test_case "match_ global collects all full matches" `Quick
            test_match_global_collects_all_full_matches;
          Alcotest.test_case
            "match_ global empty advances with unicode semantics" `Quick
            test_match_global_empty_advances_with_unicode_semantics;
          Alcotest.test_case "match_instance global resets lastIndex" `Quick
            test_match_instance_global_resets_last_index;
          Alcotest.test_case "match_instance non-global sticky uses lastIndex"
            `Quick test_match_instance_non_global_sticky_uses_last_index;
          Alcotest.test_case "match_js returns raw UTF-16 slices" `Quick
            test_match_js_returns_raw_utf16_slices;
          Alcotest.test_case "match_js non-global exposes raw capture slices"
            `Quick test_match_js_non_global_exposes_raw_capture_slices;
          Alcotest.test_case
            "match_js global empty advances over raw surrogate pair" `Quick
