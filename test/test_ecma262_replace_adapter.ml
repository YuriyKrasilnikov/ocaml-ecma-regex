...
let id clause number = Printf.sprintf "ecma262-%s-%04d" clause number
...
let test_product_surface_replace_rows () =
  ...
  check_ids "replace non-applicable rows" expected_nonapp_rows
    replace_nonapp_rows;
  List.iter
    (fun row ->
      ...
      Alcotest.(check string)
        "replace adapter test artifact" "test/test_ecma262_replace_adapter.ml"
        (field "next_test_artifact" row);
      Alcotest.(check string)
        "replace adapter public API status" "current_public_api"
        (field "public_api_status" row))
    replace_adapter_rows;
  List.iter
    (fun row ->
      Alcotest.(check string)
        "replace non-applicable artifact" "none"
        (field "ocaml_artifact" row);
      Alcotest.(check string)
        "replace non-applicable status" "not_ocaml_surface"
        (field "public_api_status" row))
    replace_nonapp_rows
...
let test_replace_instance_last_index_semantics () =
  ...
  Alcotest.(check string)
    "global replace resets lastIndex before matching" "XbX"
    (Ecma_regex.replace_instance ~replacement:"X" global_instance "aba");
  Alcotest.(check int)
    "global replace leaves terminal lastIndex" 0
...
let test_replace_all_explicit_helper () =
  ...
  Alcotest.(check string)
    "replace_all_instance preserves receiver" "bXXX"
    (Ecma_regex.replace_all_instance ~replacement:"X" instance "baaa");
  Alcotest.(check int)
    "replace_all_instance does not mutate receiver" 2
...
let test_replace_js_preserves_raw_utf16_slices () =
  ...
  Alcotest.(check (list int))
    "JS unicode empty replaceAll units"
    [ Char.code '-'; high; low; Char.code '-'; Char.code 'a'; Char.code '-' ]
    replaced

let () =
  Alcotest.run "ecma262-replace-adapter"
    [
      ( "manifest",
        [
          Alcotest.test_case "product-surface replace rows" `Quick
            test_product_surface_replace_rows;
        ] );
      ( "adapter semantics",
        [
          Alcotest.test_case "replace first and global modes" `Quick
            test_replace_first_and_global_modes;
          Alcotest.test_case "replace instance lastIndex semantics" `Quick
            test_replace_instance_last_index_semantics;
          Alcotest.test_case "GetSubstitution special references" `Quick
            test_get_substitution_special_references;
          Alcotest.test_case "GetSubstitution numbered captures" `Quick
            test_get_substitution_numbered_capture_edges;
          Alcotest.test_case "GetSubstitution named captures" `Quick
            test_get_substitution_named_captures;
          Alcotest.test_case "replace_all explicit helper" `Quick
            test_replace_all_explicit_helper;
          Alcotest.test_case "replace_js preserves raw UTF-16 slices" `Quick
