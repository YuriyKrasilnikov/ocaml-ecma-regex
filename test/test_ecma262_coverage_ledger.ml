let repo_root () =
  ...
  let rec climb dir =
    let candidate =
      Filename.concat dir "cache/ecma262-regexp-coverage-ledger.tsv"
    in
...
let ledger_rows () = read_tsv [ "cache"; "ecma262-regexp-coverage-ledger.tsv" ]
...
let test_coverage_ledger_manifest () =
  ...
  List.iter
    (fun row ->
      if String.starts_with ~prefix:"covered_by_" (field "ledger_state" row)
      then begin
        Alcotest.(check string)
          "covered bucket" "covered"
          (field "ledger_bucket" row);
        Alcotest.(check string)
          "covered release gate" "not_blocking" (field "release_gate" row);
        let expected_owner =
          match field "ledger_state" row with
          | "covered_by_test262_literal_lexer_exact" ->
              "test262_executable_extractor"
          | "covered_by_search_adapter" | "covered_by_match_adapter"
          | "covered_by_match_all_adapter" | "covered_by_split_adapter"
          | "covered_by_replace_adapter" | "covered_by_escape_adapter" ->
              "product_surface_adapter"
          | "covered_by_ucd_generated_tests" -> "ucd_generated_tests"
          | _ -> "local_exact_tests"
        in
        Alcotest.(check string)
          "covered owner" expected_owner
          (field "coverage_owner" row);
        Alcotest.(check string)
          "covered next artifact" "none"
          (field "ledger_next_artifact" row);
        if field "ledger_state" row = "covered_by_search_adapter" then begin
          Alcotest.(check string)
            "search adapter surface" "search_adapter" (field "surface_area" row);
          Alcotest.(check string)
          ...
          Alcotest.(check string)
            "match adapter surface" "match_adapter" (field "surface_area" row);
          Alcotest.(check string)
            "match adapter artifact" "Ecma_regex.match_"
            (field "ocaml_artifact" row);
          Alcotest.(check string)
          ...
          Alcotest.(check string)
            "matchAll adapter surface" "match_all_adapter"
            (field "surface_area" row);
          Alcotest.(check string)
            "matchAll adapter artifact" "Ecma_regex.match_all"
            (field "ocaml_artifact" row);
          Alcotest.(check string)
          ...
          Alcotest.(check string)
            "split adapter surface" "split_adapter" (field "surface_area" row);
          Alcotest.(check string)
            "split adapter artifact" "Ecma_regex.split"
            (field "ocaml_artifact" row);
          Alcotest.(check string)
          ...
          Alcotest.(check string)
            "replace adapter surface" "replace_adapter"
            (field "surface_area" row);
          Alcotest.(check string)
            "replace adapter artifact" "Ecma_regex.replace"
            (field "ocaml_artifact" row);
          Alcotest.(check string)
          ...
          Alcotest.(check string)
            "escape adapter surface" "regexp_escape_adapter"
            (field "surface_area" row);
          Alcotest.(check string)
          ...
          Alcotest.(check string)
            "UCD generated route" "needs_ucd_generated_tests"
        ...
        else if field "ledger_state" row = "covered_by_compile_parser_exact"
        then begin
          Alcotest.(check string)
            "compile/parser exactness evidence kind" "compile_parser_exact_case"
            (field "exactness_evidence_kind" row);
          Alcotest.(check string)
          ...
          then
            Alcotest.failf
              "%s: covered row must have compile-parser-exact case id"
              (field "requirement_id" row)
        end
        else if field "ledger_state" row = "covered_by_literal_lexer_exact" then begin
          Alcotest.(check string)
            "literal lexer exactness evidence kind" "literal_lexer_exact_case"
            (field "exactness_evidence_kind" row);
          Alcotest.(check string)
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have \
                 match-engine-unicode-sets-string-exact case id"
                (field "requirement_id" row)
            end
          else if
            field "exactness_evidence_kind" row
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have \
                 match-engine-unicode-sets-escape-string-exact case id"
                (field "requirement_id" row)
            end
          else if
            field "exactness_evidence_kind" row
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have \
                 match-engine-character-classes-exact case id"
                (field "requirement_id" row)
            end
          else if
            field "exactness_evidence_kind" row
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have match-engine-concatenation-exact \
                 case id"
                (field "requirement_id" row)
            end
          else if
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have match-engine-backreference-exact \
                 case id"
                (field "requirement_id" row)
            end
          else if
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have \
                 match-engine-backreference-matcher-exact case id"
                (field "requirement_id" row)
            end
          else if
            field "exactness_evidence_kind" row
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have match-engine-start-anchor-exact \
                 case id"
                (field "requirement_id" row)
            end
          else if
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have match-engine-end-anchor-exact case \
                 id"
                (field "requirement_id" row)
            end
          else if
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have match-engine-quantifier-exact case \
                 id"
                (field "requirement_id" row)
            end
          else if
          ...
          then
            ...
            then
              Alcotest.failf
                "%s: covered row must have \
                 match-engine-pattern-semantics-exact case id"
                (field "requirement_id" row)
            end
          else if
            field "exactness_evidence_kind" row
          ...
          field "ledger_state" row = "covered_by_local_exact_compile_parser"
        then begin
          Alcotest.(check string)
            "local exactness evidence kind" "local_exact_compile_parser_case"
            (field "exactness_evidence_kind" row);
          Alcotest.(check string)
          ...
          if
            not
              (String.starts_with ~prefix:"local-exact:"
                 (field "exactness_case_id" row))
          then
            Alcotest.failf "%s: covered row must have local-exact case id"
              (field "requirement_id" row)
        end
        else if field "ledger_state" row = "covered_by_exec_result_exact" then begin
          Alcotest.(check string)
            "exec-result exactness evidence kind" "true"
            (string_of_bool
               (field "exactness_evidence_kind" row
          ...
          then
            ...
            field "exactness_evidence_kind" row = "exec_result_exec_exact_case"
          then
            begin if
              not
          ...
          then
            ...
            Alcotest.failf "%s: unsupported exec-result evidence kind %s"
              (field "requirement_id" row)
              (field "exactness_evidence_kind" row)
        end
        else if field "ledger_state" row = "covered_by_spec_model_exact" then begin
          Alcotest.(check string)
            "spec-model exactness evidence kind" "spec_model_exact_case"
            (field "exactness_evidence_kind" row);
          Alcotest.(check string)
            "spec-model exactness credit" "spec_model_exact_requirement_credit"
            (field "exactness_coverage_credit" row);
          if
            not
              (String.starts_with ~prefix:"spec-model-exact:"
                 (field "exactness_case_id" row))
          then
            Alcotest.failf "%s: covered row must have spec-model-exact case id"
              (field "requirement_id" row)
        end
        else if
          field "ledger_state" row = "covered_by_test262_literal_lexer_exact"
        then begin
          Alcotest.(check string)
          ...
          then
            Alcotest.failf "%s: covered row must have test262 case source"
              (field "requirement_id" row)
        end
        else begin
          Alcotest.(check string)
          ...
          if
            not
              (String.starts_with ~prefix:"reused-exact:"
                 (field "exactness_case_id" row))
          then
            Alcotest.failf "%s: covered row must have reused-exact case id"
              (field "requirement_id" row)
        end;
        if
...
let () =
  Alcotest.run "ecma262-coverage-ledger"
    [
      ( "manifest",
        [
          Alcotest.test_case "coverage ledger counts and exactness overlay"
            `Quick test_coverage_ledger_manifest;
        ] );
    ]
