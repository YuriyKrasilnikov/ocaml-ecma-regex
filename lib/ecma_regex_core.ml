(** Internal implementation used by the public Ecma_regex facade and exact
    tests. *)
...
type regexp_modifiers = { add_modifiers : string; remove_modifiers : string }
...
and ast = Disjunction of atom list list
and quantifier = { prefix : quantifier_prefix; greedy : bool }
...
type t = Compiled of string * flags * ast
type instance = { regexp : t; mutable last_index : int }
...
let flags ?(has_indices = false) ?(global = false) ?(ignore_case = false)
    ?(multiline = false) ?(dot_all = false) ?(unicode = false)
    ?(unicode_sets = false) ?(sticky = false) () =
  {
    has_indices;
    global;
    ignore_case;
...
let flags_of_string source =
  ...
  let rec loop index parsed =
    if index = length then Ok parsed
      ...
      | other -> Error (Printf.sprintf "unknown RegExp flag: %c" other)
  in
...
let parse_unicode_braced_escape flags pattern index =
  ...
  then None
    ...
    let rec loop cursor value saw_digit =
      if cursor >= length then Error "unterminated braced Unicode escape"
      else
...
let parse_unicode_fixed_escape flags pattern index =
  let length = String.length pattern in
  if
    index + 1 >= length || pattern.[index] <> '\\' || pattern.[index + 1] <> 'u'
  then None
  else
    let has_four_digits =
      index + 5 < length
      ...
      let rec loop cursor end_index value =
        if cursor = end_index then Ok (end_index, Literal_code_point value)
          ...
          | None ->
              Error
                (Printf.sprintf
                   "invalid fixed Unicode escape digit at offset %d: %C" cursor
                   pattern.[cursor])
...
let parse_hex_escape pattern index =
  let length = String.length pattern in
  if
    index + 1 >= length || pattern.[index] <> '\\' || pattern.[index + 1] <> 'x'
  then None
  else if index + 3 >= length then Some (Error "unterminated hex escape")
  else
    ...
    | _ ->
        Some
          (Error (Printf.sprintf "invalid hex escape digits at offset %d" index))

let parse_control_letter_escape pattern index =
  let length = String.length pattern in
  if
    index + 2 >= length || pattern.[index] <> '\\' || pattern.[index + 1] <> 'c'
  then None
  else
...
let is_supported_unicode_binary_property_alias = function
  ...
  | "Changes_When_Uppercased" | "CWU" | "Dash" | "Default_Ignorable_Code_Point"
  | "DI" | "Deprecated" | "Dep" | "Diacritic" | "Dia" | "Emoji"
  ...
  | "Soft_Dotted" | "SD" | "Terminal_Punctuation" | "Term" | "Unified_Ideograph"
  | "UIdeo" | "Uppercase" | "Upper" | "Variation_Selector" | "VS"
...
let is_supported_unicode_property_escape_body flags ~negated body =
  ...
  || flags.unicode_sets && (not negated)
        ...
        if index + 2 >= length || pattern.[index + 2] <> '{' then
          ...
          let rec loop cursor =
            if cursor >= length then
              ...
              | '}' ->
                  let body =
                    String.sub pattern (index + 3) (cursor - index - 3)
                  in
                  if
                    is_supported_unicode_property_escape_body flags
                      ~negated:(prefix = 'P') body
                  then
                    Ok
                      ( cursor + 1,
              ...
              | char -> (
                  ...
                  | _ ->
                      Error
                        (Printf.sprintf
                           "invalid RegExp Unicode property escape character \
                            at offset %d: %C"
                           cursor char))
          in
          ignore prefix;
...
let parse_capture_name pattern index =
  ...
  if index >= length then Error "unterminated RegExp group name"
    ...
    if not (is_ascii_identifier_start first) then
      Error
        (Printf.sprintf "invalid RegExp group name start at offset %d: %C" index
           first)
    else
      let rec loop cursor =
        if cursor >= length then Error "unterminated RegExp group name"
          ...
          | char ->
              Error
                (Printf.sprintf
                   "invalid RegExp group name character at offset %d: %C" cursor
                   char)
      in
      loop (index + 1)
    ...
    | '1' .. '9' ->
        ...
        Some
          (Ok
             ( next_index,
               Decimal_escape
                 (String.sub pattern (index + 1) (next_index - index - 1)) ))
    | _ -> None

let parse_optional_lazy_suffix pattern index =
  if index < String.length pattern && pattern.[index] = '?' then
    (index + 1, false)
  else (index, true)
...
let reject_repeated_quantifier pattern index =
  if has_quantifier_at pattern index then
    Error "invalid RegExp repeated quantifier"
  else Ok ()
...
let parse_quantifier pattern index atom =
  if index >= String.length pattern then Ok (index, atom)
    ...
    match pattern.[index] with
    | '*' -> (
        let next_index, greedy =
          parse_optional_lazy_suffix pattern (index + 1)
        in
        match reject_repeated_quantifier pattern next_index with
        | Error _ as error -> error
        | Ok () ->
            Ok (next_index, Quantified (atom, { prefix = Zero_or_more; greedy }))
        )
    | '+' -> (
        let next_index, greedy =
          parse_optional_lazy_suffix pattern (index + 1)
        ...
        | Error _ as error -> error
        | Ok () ->
            Ok (next_index, Quantified (atom, { prefix = One_or_more; greedy }))
        )
    | '?' -> (
        let next_index, greedy =
          parse_optional_lazy_suffix pattern (index + 1)
        ...
        | Error _ as error -> error
        | Ok () ->
            Ok (next_index, Quantified (atom, { prefix = Zero_or_one; greedy }))
        )
    | '{' -> (
        match parse_braced_quantifier pattern index with
        | Some (after_prefix, prefix) -> (
            let next_index, greedy =
              parse_optional_lazy_suffix pattern after_prefix
            in
            match reject_repeated_quantifier pattern next_index with
            | Error _ as error -> error
                  ...
                  | None -> (
                      ...
                      | None -> (
                          ...
                          if index + 1 >= length then
                            Error "unterminated RegExp escape"
                          else
                            ...
                            | 'k'
                              when index + 2 < length
                                   && pattern.[index + 2] = '<' -> (
                                match
                                  parse_capture_name pattern (index + 3)
                                with
                                | Error _ as error -> error
                                | Ok (next_index, name) ->
                                    Ok (next_index, Named_backreference name))
                            | 'c' when not (flags.unicode || flags.unicode_sets)
                              ->
                                Ok
                                  ( index + 1,
                                    Literal_code_point (Char.code '\\') )
                            | 'c' ->
                                Error
                                  "invalid RegExp Unicode control-letter escape"
                            | char when is_escaped_syntax_char char ->
                                Ok
                                  ( index + 2,
                                    Literal_code_point (Char.code char) )
                            | '-' when not (flags.unicode || flags.unicode_sets)
                              ->
                                Ok
                                  (index + 2, Literal_code_point (Char.code '-'))
                            ...
                            | char ->
                                Error
                                  (Printf.sprintf
                                     "unsupported RegExp escape at offset %d: \
                                      \\\\%c"
                                     index char)))))))
...
let parse_class_string_disjunction_escape pattern index =
  ...
  then
    let rec loop cursor =
      if cursor >= length then
        Error "unterminated RegExp class string disjunction"
...
let unicode_property_escape_may_contain_strings source =
  ...
  if
    ...
    && source.[length - 1] = '}'
  then
    is_supported_unicode_string_property_name (String.sub source 3 (length - 4))
  else false
...
let parse_class_escape flags pattern index =
  ...
  then Ok (index + 2, false)
    ...
    index + 1 < length && pattern.[index] = '\\' && pattern.[index + 1] = 'c'
  then
...
let rec parse_atom flags pattern index =
  ...
  | char -> (
      if is_unsupported_syntax_char char then
        Error
          (Printf.sprintf "unsupported RegExp syntax at offset %d: %C" index
             char)
      else
...
and parse_regular_expression_modifiers pattern index =
  ...
  let add_modifier index char modifiers =
    if List.exists (( = ) char) modifiers then
      Error
        (Printf.sprintf "duplicate RegExp modifier at offset %d: %C" index char)
  ...
  let rec add_loop cursor saw_modifier modifiers =
    if cursor >= length then Error "unterminated RegExp modifiers group"
      ...
      | char ->
          Error
            (Printf.sprintf "invalid RegExp modifier at offset %d: %C" cursor
               char)
    ...
    if cursor >= length then Error "unterminated RegExp modifiers group"
      ...
      | ('i' | 'm' | 's') as char -> (
          if List.exists (( = ) char) add_modifiers then
            Error
              (Printf.sprintf
                 "RegExp modifier appears in both add and remove lists at \
                  offset %d: %C"
                 cursor char)
          else
    ...
    if index + 2 >= length then Error "unterminated RegExp group prefix"
      ...
      | 'i' | 'm' | 's' | '-' -> (
          ...
          | Ok (body_index, modifiers) -> (
              ...
              | Error _ as error -> error
              | Ok (next_index, ast) ->
                  Ok (next_index, Modifiers_group (modifiers, ast))))
      | char ->
          Error
            (Printf.sprintf "unsupported RegExp group prefix at offset %d: ?%c"
               index char)
  else
    match
      parse_disjunction_until flags pattern (index + 1) ~stop_on_close:true
        ~allow_empty_alternative:true
    with
    | Error _ as error -> error
...
and parse_disjunction_until flags pattern start_index ~stop_on_close
  ...
  let rec loop index alternatives atoms =
    if index = length then
      ...
      | ')' ->
          Error
            (Printf.sprintf "unsupported RegExp syntax at offset %d: ')'" index)
      | '|' -> (
          match push_alternative index alternatives atoms with
          | Error _ as error -> error
      ...
      | _ -> (
          ...
          | Ok (atom_end_index, atom) -> (
              ...
              | Error _ as error -> error
              | Ok (next_index, atom) ->
                  loop next_index alternatives (atom :: atoms)))
  in
  loop start_index [] []
...
let parse_disjunction flags pattern =
  let length = String.length pattern in
  match
    parse_disjunction_until flags pattern 0 ~stop_on_close:false
      ~allow_empty_alternative:true
  with
  | Error _ as error -> error
  ...
  | Ok (next_index, _) ->
      Error
        (Printf.sprintf "unparsed RegExp syntax remains at offset %d" next_index)
              ...
              | path :: remaining_paths ->
                  let rec combine_atom_paths acc = function
                    ...
                    | atom_path :: remaining_atom_paths -> (
                        ...
                        | Error _ as error -> error
                        | Ok combined ->
                            combine_atom_paths (combined :: acc)
                              remaining_atom_paths)
                  in
...
and capture_name_paths_atom = function
  | Literal_code_point _ | Dot | Character_class _ | Character_class_escape _
  | Start_anchor | End_anchor | Assertion_escape _ | Unicode_property_escape _
  | Named_backreference _ | Decimal_escape _ ->
      Ok [ [] ]
...
let rec capture_indices_ast = function
  | Disjunction alternatives ->
      List.fold_left
        (fun acc atoms ->
          List.rev_append (capture_indices_alternative atoms) acc)
        [] alternatives
      |> List.rev
...
let skip_character_class_escape_source source index =
  ...
  if
    index + 2 < length
    && source.[index] = '\\'
    && (match source.[index + 1] with 'p' | 'P' -> true | _ -> false)
    && source.[index + 2] = '{'
  then
    let rec loop cursor =
...
let is_unicode_sets_intersection_operator source index =
  index > 1 && index + 2 < String.length source - 1
...
let validate_unicode_sets_character_class_syntax source =
  ...
  let rec loop index =
    ...
    else if index + 1 < length && String.equal (String.sub source index 2) "&&"
    then
...
let validate_character_class_range flags source =
  ...
  let validate_unicode_sets () =
    if flags.unicode_sets then
      validate_unicode_sets_character_class_syntax source
    else Ok ()
  in
...
let rec validate_quantifier_ranges_ast flags = function
  | Disjunction alternatives ->
      validate_quantifier_ranges_alternatives flags alternatives
...
and validate_quantifier_ranges_atom flags = function
  | Quantified
      ( ( Positive_lookahead _ | Negative_lookahead _ | Positive_lookbehind _
  ...
  | Literal_code_point _ | Dot | Character_class_escape _ | Start_anchor
  | End_anchor | Assertion_escape _ | Unicode_property_escape _
  | Named_backreference _ | Decimal_escape _ ->
      Ok ()
      ...
      let next, alternatives =
        List.fold_left
          (fun (next, alternatives) atoms ->
            let next, atoms =
              assign_capture_indices_in_alternative next atoms
            in
...
and assign_capture_indices_in_atom next = function
  ...
  | ( Literal_code_point _ | Dot | Character_class _ | Character_class_escape _
    | Start_anchor | End_anchor | Assertion_escape _ | Unicode_property_escape _
    | Named_backreference _ | Decimal_escape _ ) as atom ->
      (next, atom)
          ...
          | Ok () -> (
              ...
              | Ok () -> (
                  ...
                  | Ok () ->
                      Ok
                        (Compiled
                           (pattern, flags, assign_capture_indices_ast ast))))))
...
type match_trace = {
  ...
  mutable trace_character_complement_allcharacters_case_fold_stable_universe :
    bool;
...
type capture_range = { range_start_index : int; range_end_index : int }
type match_direction = Forward | Backward
...
let match_sequence_observation_of_trace trace =
  {
    match_sequence_operation_observed = trace.trace_match_sequence_operation;
...
let character_class_observation_of_trace trace =
  {
    character_range_operation_observed = trace.trace_character_range_operation;
    ...
    character_range_start_code_observed = trace.trace_character_range_start_code;
    character_range_end_code_observed = trace.trace_character_range_end_code;
...
let capture_observation_of_trace trace =
  {
    ...
    capture_subpattern_matcher_observed = trace.trace_capture_subpattern_matcher;
    ...
    capture_backward_direction_observed = trace.trace_capture_backward_direction;
    ...
    capture_outer_continuation_observed = trace.trace_capture_outer_continuation;
...
let backreference_observation_of_trace trace =
  {
    decimal_backreference_atom_observed = trace.trace_decimal_backreference_atom;
    decimal_capturing_group_number_observed =
      trace.trace_decimal_capturing_group_number;
    decimal_group_count_assert_observed = trace.trace_decimal_group_count_assert;
    ...
    named_paren_indices_list_observed = trace.trace_named_paren_indices_list;
    ...
    named_paren_index_append_observed = trace.trace_named_paren_index_append;
...
let backreference_matcher_observation_of_trace trace =
  {
    ...
    backreference_ns_iteration_observed = trace.trace_backreference_ns_iteration;
...
let observe_character_complement_operation trace =
  match trace with
  | None -> ()
  | Some trace -> trace.trace_character_complement_operation <- true
...
let observe_unicode_sets_string_element_model
    ?(character_class_invert_assert = false) state =
  match state.trace with
  | None -> ()
  | Some trace ->
...
let observe_capture_nested_continuation direction capture_index state y cap
    range =
  match state.trace with
  | None -> ()
  | Some trace ->
      ...
      | Forward ->
          ...
          trace.trace_capture_forward_order <-
            state.index <= y.index
            && range.range_start_index <= range.range_end_index;
...
let any_range_matches ranges char = List.exists (in_range char) ranges
...
let parse_simple_class_element source stop index =
  if index >= stop then unsupported_match_engine "empty character class element"
    ...
    if index + 1 >= stop then
      unsupported_match_engine "unterminated class escape"
    else
      ...
      | None -> (
          ...
          | 'D' | 'S' | 'W' ->
              unsupported_match_engine
                "negative class escape inside character class"
...
let ranges_of_character_class ?trace flags source =
  ...
  let annex_b_range_or_union first extra_ranges ranges =
    ...
    else List.rev_append extra_ranges ((first, first) :: ('-', '-') :: ranges)
  in
    ...
    if index >= stop then (inverted, List.rev ranges)
      ...
      | Class_char first
        when next_index < stop
          ...
          | Class_ranges extra_ranges ->
              loop after_range
                (annex_b_range_or_union first extra_ranges ranges))
...
let has_unicode_semantics flags = flags.unicode || flags.unicode_sets
...
let character_class_matches_code_unit ?trace flags source code_unit =
  ...
  if inverted then begin
    observe_character_complement_operation trace;
    let in_universe =
      allcharacters_contains_code_point ?trace flags code_unit
    in
    let result = in_universe && not member in
    observe_character_complement_difference trace;
    result
...
let rgi_emoji_grinning_face = "\xF0\x9F\x98\x80"
...
let unicode_sets_string_element_matcher_of_elements direction flags
    ~character_class_invert_assert elements =
  if flags.ignore_case then
  ...
  in
  let singles =
    List.filter (fun element -> String.length element = 1) elements
  in
  let has_empty =
    List.exists (fun element -> String.length element = 0) elements
  in
  ...
  fun state continuation ->
    observe_unicode_sets_string_element_model ~character_class_invert_assert
      state;
...
let unicode_sets_string_element_matcher direction flags source =
  ...
  unicode_sets_string_element_matcher_of_elements direction flags
    ~character_class_invert_assert:true elements
...
let unicode_sets_escape_string_element_matcher direction flags source =
  ...
  | Some elements ->
      unicode_sets_string_element_matcher_of_elements direction flags
        ~character_class_invert_assert:false elements
...
let js_string_of_utf16_code_units code_units =
  ...
  | Some unit ->
      Error (Printf.sprintf "UTF-16 code unit out of range: 0x%X" unit)
...
let regexp_escape_is_syntax_character = function
  | 0x5E | 0x24 | 0x5C | 0x2E | 0x2A | 0x2B | 0x3F | 0x28 | 0x29 | 0x5B | 0x5D
  | 0x7B | 0x7D | 0x7C ->
      true
  | _ -> false
...
let regexp_escape_is_other_punctuator = function
  | 0x2C | 0x2D | 0x3D | 0x3C | 0x3E | 0x23 | 0x26 | 0x21 | 0x25 | 0x3A | 0x3B
  | 0x40 | 0x7E | 0x27 | 0x60 | 0x22 ->
      true
  | _ -> false
...
let regexp_escape_encode_code_point add_code_point buffer code_point =
  if regexp_escape_is_syntax_character code_point || code_point = 0x2F then begin
    ...
    | None ->
        ...
        then
          begin if code_point <= 0xFF then
            regexp_escape_add_x_escape buffer code_point
          else
            List.iter
...
let utf16_code_unit_width_at_index value code_unit_index =
  let byte_index =
    utf8_byte_index_of_utf16_code_unit_index value code_unit_index
  in
...
let text_of_capture_range input = function
  ...
  | Some range ->
      Some
        (String.sub input range.range_start_index
...
let consume_code_point_if_matches ~unicode direction state continuation
    predicate =
  match direction with
  | Forward -> (
...
let unicode_character_property_escape_matches source code_point =
  ...
  let matches =
    ...
    | [ value ] -> (
        match
          Ecma_regex_ucd_tables.general_category_value_canonical_name value
        with
        | Some _ ->
            Ecma_regex_ucd_tables.general_category_matches value code_point
        | None -> (
            match
              Ecma_regex_ucd_tables.binary_property_canonical_name value
            with
            | Some _ ->
                Ecma_regex_ucd_tables.binary_property_matches value code_point
            | None ->
                unsupported_match_engine ("Unicode property escape " ^ source)))
...
let ensure_class_escape_within_stop source stop next_index =
  if next_index <= stop then ()
    ...
    unsupported_match_engine ("unterminated character class escape in " ^ source)
...
let parse_unicode_class_element flags source stop index =
  if index >= stop then unsupported_match_engine "empty character class element"
    ...
    if index + 1 >= stop then
      ...
      match
        parse_legacy_unicode_class_control_escape flags source stop index
      with
      | Some result -> result
      | None -> (
          ...
          | Ok (next_index, Character_class_escape escape_source) -> (
              ...
              | "\\D" | "\\S" | "\\W" ->
                  ( next_index,
                    Unicode_class_predicate
                      (unicode_character_class_escape_matches flags
                         escape_source) )
              | _ ->
                  unsupported_match_engine
                    ("character class escape " ^ escape_source))
      ...
      | Unicode_class_code_point first
        when next_index < stop
          ...
          | Unicode_class_code_point last when first <= last ->
              observe_character_range trace;
              loop after_range
                (Unicode_class_ranges [ (first, last) ] :: elements)
          | Unicode_class_code_point _ ->
              unsupported_match_engine "invalid character class range"
          | Unicode_class_ranges _ | Unicode_class_predicate _ ->
              unsupported_match_engine "class escape as range endpoint")
...
let unicode_character_class_matches ?trace flags source code_point =
  let inverted, elements =
    unicode_character_class_elements ?trace flags source
  in
  let member =
    List.exists
      (fun element ->
...
let end_anchor_matches flags state =
  ...
  || flags.multiline && state.index < input_length
     &&
     match utf8_decode_next state.input state.index with
     | Some (_, code_unit) -> is_line_terminator_code_point code_unit
     | None -> false
...
let string_contains_char value char = String.exists (( = ) char) value
...
let apply_regexp_modifiers flags modifiers =
  {
    flags with
    ignore_case = update_modifier_flag modifiers 'i' flags.ignore_case;
    multiline = update_modifier_flag modifiers 'm' flags.multiline;
    dot_all = update_modifier_flag modifiers 's' flags.dot_all;
  }
...
let match_backreference_indices direction flags indices state continuation =
  ...
  let rec first_defined = function
    ...
    | capture_index :: rest -> (
        ...
        if capture_index < 0 || capture_index >= Array.length state.captures
        then unsupported_match_engine "backreference capture index"
        else
...
let parse_braced_bounds source =
  ...
  if length < 3 || source.[0] <> '{' || source.[length - 1] <> '}' then
    ...
    | [ exact ] ->
        ...
        (count, Some count)
    | [ minimum; "" ] -> (int_of_string minimum, None)
    | [ minimum; maximum ] ->
...
let bounds_of_quantifier quantifier =
  ...
  in
  { min_repetitions; max_repetitions; repeat_greedy = quantifier.greedy }

let repeat_max_allows_more max_repetitions count =
  match max_repetitions with None -> true | Some max -> count < max
...
let capture_range_for direction x y =
  match direction with
  | Forward -> { range_start_index = x.index; range_end_index = y.index }
  | Backward -> { range_start_index = y.index; range_end_index = x.index }
...
let rec matcher_of_atom direction flags named_indices capture_count = function
  ...
  | Character_class source ->
      if
        flags.unicode_sets
        && character_class_contains_class_string_disjunction source
      ...
      else if has_unicode_semantics flags then fun state continuation ->
        consume_code_point_if_matches ~unicode:true direction state continuation
  ...
  | Named_capture_group (_, capture_index, ast) ->
      ...
      fun state continuation ->
        ...
        let nested_continuation y =
          ...
          observe_capture_nested_continuation direction capture_index state y
            cap range;
          let z =
            {
              input = state.input;
  ...
  | Unicode_property_escape source ->
      if
        flags.unicode_sets && unicode_property_escape_may_contain_strings source
      then unicode_sets_escape_string_element_matcher direction flags source
      else fun state continuation ->
        consume_code_point_if_matches
          ~unicode:(has_unicode_semantics flags)
          direction state continuation
  ...
  | Modifiers_group (modifiers, ast) ->
      ...
      matcher_of_disjunction direction modified_flags named_indices
        capture_count ast
  | Quantified (atom, quantifier) ->
      let atom_matcher =
      ...
      let rec repeat count state continuation =
        ...
        let try_more () =
          if not (repeat_max_allows_more bounds.max_repetitions count) then None
            ...
            in
            let repeat_state =
              if captures == state.captures then state
              else { state with captures }
            in
            atom_matcher repeat_state (fun next_state ->
                if next_state.index = state.index then
                  let next_count = count + 1 in
                  if next_count >= bounds.min_repetitions then
                    continuation next_state
...
and matcher_of_alternatives direction flags named_indices capture_count =
  function
  | [] -> fun _ _ -> None
  | [ atoms ] ->
  ...
  | atoms :: rest ->
      match_two_alternatives
        (matcher_of_atoms direction flags named_indices capture_count atoms)
        (matcher_of_alternatives direction flags named_indices capture_count
           rest)

and matcher_of_disjunction direction flags named_indices capture_count =
  function
  | Disjunction alternatives ->
      matcher_of_alternatives direction flags named_indices capture_count
        alternatives

let match_disjunction_state_at ?trace ?(direction = Forward) flags input index
    ast =
  ...
  in
  let state =
    { input; index; captures = Array.make capture_count None; trace }
  in
...
let inspect_spec_model ?(model_scenario = "default") source =
  ...
  in
  let observed field = Array.exists (( = ) field) observed_model_fields in
  {
...
let compile_atom_piecewise_dispatch_case_observed = function
  | Literal_code_point _ | Dot | Character_class _ | Character_class_escape _
  | Capturing_group _ | Named_capture_group _ | Noncapturing_group _
  | Positive_lookahead _ | Negative_lookahead _ | Positive_lookbehind _
  | Negative_lookbehind _ | Start_anchor | End_anchor | Assertion_escape _
  | Unicode_property_escape _ | Named_backreference _ | Decimal_escape _
  | Modifiers_group _ | Quantified _ ->
      true
...
let compile_atom_matcher_constructible flags ast atom =
  ...
  try
    ...
    in
    true
  with Unsupported_match_engine _ -> false
...
let inspect_match_sequence_model (Compiled (_source, flags, ast)) input =
  ...
  ignore (exec_ast ~trace flags ast input);
  ignore
    (match_disjunction_at ~trace ~direction:Backward flags input
       (String.length input) ast);
...
let inspect_unicode_sets_string_element_model (Compiled (_source, flags, ast))
    input =
  let trace = fresh_match_trace () in
  ignore (exec_ast ~trace flags ast input);
...
let capture_names_ast ast =
  ...
  and fill_alternative atoms = List.iter fill_atom atoms
...
let js_match_result_of_state input ast start_index end_index state =
  let result = match_result_of_byte_range input start_index end_index in
  {
    ...
    js_named_captures = js_named_captures_of_ranges input ast state.captures;
  }
...
let array_exists predicate array =
  let rec loop index =
    index < Array.length array && (predicate array.(index) || loop (index + 1))
  in
  loop 0
...
let add_many fields acc = List.fold_right add_unique fields acc
...
let safe_exec_ast_state flags ast input =
  try exec_ast_state flags ast input with Unsupported_match_engine _ -> None
...
let inspect_pattern_semantics_model ?(model_scenario = "default")
  ...
  let exec_result =
    safe_exec_ast_state flags ast input |> exec_result_of_state input
  in
  ...
  {
    observed_model_fields =
      add_many model_fields [] |> add_many unicode_fields
      |> add_many notation_fields |> add_many record_fields
...
let inspect_exec_result_matching_model ?(model_scenario = "default_match")
  ...
  {
    observed_model_fields =
      add_many base_fields [] |> add_many result_fields |> add_many group_fields
      |> add_many scenario_fields |> List.rev |> Array.of_list;
    exec_result;
    input_length;
    last_index_before;
    last_index_after;
  ...
  | Some (start_index, end_index, state) ->
      ...
      in
      {
        ...
        index_pair_range_valid =
          Array.for_all
            (index_pair_range_valid (String.length input))
            exec_result_indices;
        ...
        indices_length_within_array_limit =
          Array.length exec_result_indices >= 0;
        ...
        indices_groups_object_created =
          has_groups && has_indices_branch_observed;
...
let exec (Compiled (_source, flags, ast)) input =
  try exec_ast flags ast input
  with Unsupported_match_engine construct ->
    invalid_arg (unsupported_match_engine_message "Ecma_regex.exec" construct)

let search (Compiled (_source, flags, ast)) input =
  try match exec_ast flags ast input with Some _ -> true | None -> false
  with Unsupported_match_engine construct ->
...
let exec_js (Compiled (_source, flags, ast)) (Js_string input) =
  ...
  with Unsupported_match_engine construct ->
    invalid_arg
      (unsupported_match_engine_message "Ecma_regex.exec_js" construct)

let search_js (Compiled (_source, flags, ast)) (Js_string input) =
  try match exec_ast flags ast input with Some _ -> true | None -> false
  with Unsupported_match_engine construct ->
    invalid_arg
      (unsupported_match_engine_message "Ecma_regex.search_js" construct)
...
let exec_instance_state operation
    ({ regexp = Compiled (_source, flags, ast); last_index } as instance) input
    =
  try
...
let match_instance ({ regexp = Compiled (_source, flags, _); _ } as instance)
  ...
  if not flags.global then
    ...
    let rec loop matches =
      match exec_instance instance input with
      | None -> if matches = [] then None else Some (List.rev matches)
      | Some result ->
          if result.matched_text = "" then begin
            let this_index = last_index instance in
            let next_index =
              advance_string_index input this_index
                (has_unicode_semantics flags)
            in
...
let match_instance_js ({ regexp = Compiled (_source, flags, _); _ } as instance)
...
let iter_matches ({ regexp = Compiled (_source, flags, _); _ } as instance)
    input =
  {
    iterating_regexp = instance;
...
let next_match_js iterator =
  if iterator.js_iter_done then None
    ...
    match
      exec_instance_js iterator.js_iterating_regexp iterator.js_iterated_string
    with
    | None ->
...
let match_all_instance instance input =
  let clone = { regexp = instance.regexp; last_index = instance.last_index } in
...
let split_limit operation = function
  | None -> max_int
  | Some limit when limit < 0 -> invalid_arg (operation ^ ": negative limit")
  | Some limit -> limit
  ...
  if limit = 0 then []
    ...
    let rec append_captures captures index acc length =
      if index >= Array.length captures then `Continue (acc, length)
        ...
        let acc, length, done_ =
          append (capture_part captures.(index)) acc length
        in
...
let split_instance ?limit instance input =
  split_instance_parts "Ecma_regex.split_instance"
    (fun text -> Split_text text)
    (fun capture -> Split_capture capture)
    ?limit instance input

let split ?limit regexp input = split_instance ?limit (instance regexp) input

let split_instance_js ?limit instance (Js_string input) =
  split_instance_parts "Ecma_regex.split_instance_js"
  ...
  let rec loop index =
    if index >= replacement_length then ()
      ...
      | '0' .. '9' ->
          ...
          else add_substring buffer replacement index (index + 1 + digit_count);
...
let replace_instance_storage operation ~replacement
    ...
    | Some (ast, start_byte, end_byte, state) ->
        ...
        if not (regexp_has_global regexp) then List.rev acc
          ...
          if start_byte = end_byte then begin
            let this_index = last_index instance in
            let next_index =
              advance_string_index input this_index unicode_matching
            in
            set_last_index instance next_index
          end;
          collect acc
  ...
  | matches ->
      let buffer =
        Buffer.create (String.length input + String.length replacement)
      in
      let rec append_matches next_source_byte = function
        ...
        | (ast, start_byte, end_byte, state, matched) :: rest ->
            if start_byte >= next_source_byte then begin
              ...
              add_get_substitution buffer input ast state.captures matched
...
let replace_instance ~replacement instance input =
  replace_instance_storage "Ecma_regex.replace_instance" ~replacement instance
    input
...
let replace_all_instance ~replacement instance input =
  let replace_all_instance =
    { regexp = regexp_with_global instance.regexp; last_index = 0 }
  in
