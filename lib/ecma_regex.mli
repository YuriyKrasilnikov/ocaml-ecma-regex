...
(** One element returned by explicit RegExp split semantics. Text elements are
    substrings that remain after removing matches. Capture elements are
    numbered captures inserted after a separator match; [None] represents an
    unmatched ECMAScript capture, i.e. JavaScript [undefined]. *)
type split_part =
...
(** [regexp_literal_of_string s] parses a complete ECMAScript
    regular-expression literal source such as ["/a/g"]. The returned
    [flag_text] is the exact source text recognized as
...
(** [replace_all ~replacement re s] replaces every match in [s] using the same
    ...
    helper over the regexp engine; it does not model JavaScript object
    dispatch for [String.prototype.replaceAll]. *)
val replace_all : replacement:string -> t -> string -> string

(** [escape s] returns ECMA-262 [RegExp.escape] pattern text for matching
    [s] literally. *)
...
(** [search_js re s] returns [true] when [re] matches explicit ECMAScript
    String [s]. *)
...
(** [replace_js ~replacement re s] is [replace] over explicit ECMAScript
    String values, preserving raw UTF-16 replacement and input slices. *)
val replace_js : replacement:js_string -> t -> js_string -> js_string

(** [replace_all_js ~replacement re s] is [replace_all] over explicit
...
(** [instance re] creates an explicit mutable RegExp instance for APIs that
    need ECMAScript [lastIndex] state. *)
...
(** [search_instance_index i s] executes search semantics through [i],
    temporarily using [lastIndex = 0] and restoring [i]'s previous
    [lastIndex] before returning. The result is the UTF-16 code-unit start
    index, or [-1] on no match. *)
...
(** [match_all_instance_js i s] is [match_all_instance] over explicit
    ECMAScript String [s]. *)
val match_all_instance_js : instance -> js_string -> js_match_result list

(** [split_instance_js ?limit i s] is [split_instance] over explicit
    ECMAScript String [s]. *)
val split_instance_js : ?limit:int -> instance -> js_string -> js_split_part list
...
(** [iter_matches i s] creates an explicit iterator over [s] using [i]'s
    mutable [lastIndex] state. Global iterators advance [lastIndex] after empty
    matches using ECMAScript [AdvanceStringIndex] rules. *)
val iter_matches : instance -> string -> match_iterator

(** [next_match it] returns the next iterator match, or [None] after
    exhaustion. *)
val next_match : match_iterator -> match_result option

(** [iter_matches_js i s] creates an explicit iterator over explicit
    ECMAScript String [s]. *)
